...
int tcl_countusers STDVAR { Context;
  BADARGS(1, 1, "");
  Tcl_AppendResult(irp, int_to_base10(count_users(userlist)), NULL);
  return TCL_OK;
} int tcl_validuser STDVAR { Context;
  BADARGS(2, 2, STR(" handle"));
  Tcl_AppendResult(irp, get_user_by_handle(userlist, argv[1]) ? "1" : "0", NULL);
  return TCL_OK;
} int tcl_finduser STDVAR { struct userrec *u;
    ...
    return TCL_OK;
} int tcl_passwdOk STDVAR { struct userrec *u;

    Context;
    BADARGS(3, 3, STR(" handle passwd"));
    Tcl_AppendResult(irp, ((u = get_user_by_handle(userlist, argv[1])) && u_pass_match(u, argv[2])) ? "1" : "0", NULL);
    return TCL_OK;
} int tcl_chattr STDVAR { char *chan,
   *chg,
    work[100];
  ...
  struct userrec *u;
    ...
    Context;
    BADARGS(2, 4, STR(" handle ?changes? ?channel?"));
  ...
  if (argc == 4) {
    ...
    if (!findchan(argv[2]) && argv[2][0] != '+') {
      ...
      Tcl_AppendResult(irp, STR("no such channel"), NULL);
      return TCL_ERROR;
...
int tcl_matchattr STDVAR { struct userrec *u;
  ...
  int ok = 0,
    ...
    Context;
    BADARGS(3, 4, STR(" handle flags ?channel?"));
    Context;			/* a2 - Last context: tcluser.c/184 */
  if ((u = get_user_by_handle(userlist, argv[1])) && ((argc == 3) || findchan(argv[3]))) {
    Context;			/* a2 - Last context: tcluser.c/184 */
    user.match = FR_GLOBAL | (argc == 4 ? FR_CHAN : 0);
...
int tcl_adduser STDVAR { Context;
  BADARGS(2, 3, STR(" handle ?hostmask?"));
...
int tcl_addbot STDVAR { struct bot_addr *bi;
  char *p,
   *q;

    Context;
    BADARGS(3, 3, STR(" handle address"));
  if (strlen(argv[1]) > HANDLEN)
      argv[1][HANDLEN] = 0;
  if (get_user_by_handle(userlist, argv[1]))
    ...
    userlist = adduser(userlist, argv[1], STR("none"), "-", USER_BOT);
    ...
    if (!q) {
      ...
      strncpy0(bi->address, argv[2], q - argv[2]+1);
      p = q + 1;
...
int tcl_deluser STDVAR { Context;
  BADARGS(2, 2, STR(" handle"));
  Tcl_AppendResult(irp, (argv[1][0] == '*') ? "0" : int_to_base10(deluser(argv[1])), NULL);
  return TCL_OK;
} int tcl_delhost STDVAR { Context;
  BADARGS(3, 3, STR(" handle hostmask"));
  if ((!get_user_by_handle(userlist, argv[1])) || (argv[1][0] == '*')) {
    Tcl_AppendResult(irp, STR("non-existent user"), NULL);
    return TCL_ERROR;
  }
...
int tcl_userlist STDVAR { struct userrec *u = userlist;
  ...
  while (u) {
    if (argc >= 2) {
      user.match = FR_GLOBAL | FR_CHAN | (argc == 3 ? 0 : FR_ANYWH);
...
#ifdef HUB
int tcl_save STDVAR { Context;
  write_userfile(-1);
  return TCL_OK;
}
#endif
#ifdef HUB
int tcl_reload STDVAR { Context;
  reload();
  return TCL_OK;
}
#endif
int tcl_chnick STDVAR { struct userrec *u;
  char newhand[HANDLEN + 1];
  int x = 1,
    i;

    Context;
    BADARGS(3, 3, STR(" oldnick newnick"));
    u = get_user_by_handle(userlist, argv[1]);
  if (!u)
      x = 0;
  else {
    strncpy0(newhand, argv[2], HANDLEN+1);
    ...
    if (strchr(BADHANDCHARS, newhand[0]) != NULL)
      ...
      x = 0;
    else if (!strcasecmp(botnetnick, newhand) && (!(u->flags & USER_BOT) || nextbot(argv[1]) != -1))
      x = 0;
    else if (newhand[0] == '*')
      x = 0;
  } if (x)
...
int tcl_getting_users STDVAR { int i;
...
int tcl_isignore STDVAR { Context;
  BADARGS(2, 2, STR(" nick!user@host"));
  Tcl_AppendResult(irp, match_ignore(argv[1]) ? "1" : "0", NULL);
  return TCL_OK;
} int tcl_newignore STDVAR { time_t expire_time;
  char ign[UHOSTLEN],
    cmt[66],
    from[HANDLEN + 1];

    Context;
    BADARGS(4, 5, STR(" hostmask creator comment ?lifetime?"));
    strncpy0(ign, argv[1], UHOSTMAX);
...
int tcl_killignore STDVAR { Context;
  BADARGS(2, 2, STR(" hostmask"));
  Tcl_AppendResult(irp, delignore(argv[1]) ? "1" : "0", NULL);
  return TCL_OK;
}
/* { hostmask note expire-time create-time creator } */ int tcl_ignorelist STDVAR {
  struct igrec *i;
  char ts[21],
    ts1[21],
   *list[5],
  ...
  for (i = global_ign; i; i = i->next) {
    list[0] = i->igmask;
    list[1] = i->msg;
    sprintf(ts, STR("%lu"), i->expire);
    list[2] = ts;
    sprintf(ts1, STR("%lu"), i->added);
    list[3] = ts1;
    list[4] = i->user;
    p = Tcl_Merge(5, list);
...
int tcl_getuser STDVAR { struct user_entry_type *et;
  struct userrec *u;
  struct user_entry *e;

    Context;
    BADARGS(3, 999, STR(" handle type"));
  if (!(et = find_entry_type(argv[2]))) {
    Tcl_AppendResult(irp, STR("No such info type: "), argv[2], NULL);
    return TCL_ERROR;
  }
  if (!(u = get_user_by_handle(userlist, argv[1]))) {
    if (argv[1][0] != '*') {
      Tcl_AppendResult(irp, STR("No such user."), NULL);
      return TCL_ERROR;
    } else
...
int tcl_setuser STDVAR { struct user_entry_type *et;
  struct userrec *u;
  struct user_entry *e;
  int r;

    Context;
    BADARGS(3, 999, STR(" handle type ?setting....?"));
  if (!(et = find_entry_type(argv[2]))) {
    Tcl_AppendResult(irp, STR("No such info type: "), argv[2], NULL);
    return TCL_ERROR;
  }
