...
static int tcl_addbot STDVAR
{
  ...
  if (get_user_by_handle (userlist, argv[1]))
      ...
      else
	{
      ...
      set_user (&USERENTRY_BOTADDR, get_user_by_handle (userlist, argv[1]),
		bi);
      Tcl_AppendResult (irp, "1", NULL);
    }
  return TCL_OK;
...
static int tcl_deluser STDVAR
{
  ...
  Tcl_AppendResult (irp,
		    (argv[1][0] ==
		     '*') ? "0" : int_to_base10 (deluser (argv[1])), NULL);
  return TCL_OK;
}
...
static int tcl_userlist STDVAR
{
  ...
  for (u = userlist; u; u = u->next)
	...
	{
	  user.match =
	    FR_GLOBAL | FR_CHAN | FR_BOT | (argc == 3 ? 0 : FR_ANYWH);
	  if (argc == 3)
...
static int tcl_setuser STDVAR
{
  ...
  if (!e->u.list)
    {
      if (list_delete
	  ((struct list_type **) &(u->entries), (struct list_type *) e))
	nfree (e);
    }
  return r;
}
tcl_cmds tcluser_cmds[] = { {"countusers", tcl_countusers}
