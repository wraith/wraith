...
void
addbot (char *who, char *from, char *next, char flag, int vernum)
{
...
void
updatebot (int idx, char *who, char share, int vernum)
{
...
int
addparty (char *bot, char *nick, int chan, char flag, int sock, char *from,
	  int *idx)
{
  int i;
  for (i = 0; i < parties; i++)
  ...
  if (parties == maxparty)
    {
      maxparty += 50;
      party =
	(party_t *) nrealloc ((void *) party, maxparty * sizeof (party_t));
    }
...
void
partystat (char *bot, int sock, int add, int rem)
{
  int i;
  for (i = 0; i < parties; i++)
    {
...
void
partysetidle (char *bot, int sock, int secs)
{
  int i;
  for (i = 0; i < parties; i++)
    {
...
int
partynick (char *bot, int sock, char *nick)
{
  char work[HANDLEN + 1];
  int i;
  for (i = 0; i < parties; i++)
...
void
partyaway (char *bot, int sock, char *msg)
{
  int i;
  for (i = 0; i < parties; i++)
    {
...
void
answer_local_whom (int idx, int chan)
{
  char format[81];
  char c, idle[40];
  ...
  for (i = 0; i < dcc_total; i++)
    if (dcc[i].type == &DCC_CHAT)
      {
	if ((chan == (-1))
	    || ((chan >= 0) && (dcc[i].u.chat->channel == chan)))
	  {
	    t = strlen (dcc[i].nick);
	    if (t > nicklen)
	      nicklen = t;
  ...
  for (i = 0; i < parties; i++)
      ...
      if ((chan == (-1)) || ((chan >= 0) && (party[i].chan == chan)))
	{
	  ...
	  if (party[i].timer == 0L)
	    ...
	    idle[0] = 0;
	  total++;
	  dprintf (idx, format, c, party[i].nick, (party[i].chan == 0)
		   && (chan == (-1)) ? '+' : ' ',
#ifdef HUB
...
void
tell_bottree (int idx, int showver)
{
  char s[161];
  ...
  if (s[0])
    dprintf (idx, "(%s %s)\n", BOT_NOTRACEINFO, s);
  if (showver)
    dprintf (idx, "%s (%d.%d.%d.%d)\n", botnetnick, egg_numver / 1000000,
	     egg_numver % 1000000 / 10000, egg_numver % 10000 / 100,
	     egg_numver % 100);
  else
    dprintf (idx, "%s\n", botnetnick);
  this = (tand_t *) 1;
		  ...
		  if (bot->ver)
		    {
		      ...
		      if (showver)
			sprintf (s + i, " (%d.%d.%d.%d)", bot->ver / 1000000,
		  ...
		  if (bot->uplink == last[lev - 1])
		      ...
		      else if (ok)
			{
			  ...
			  if (cnt == 1)
				...
				{
				  i =
				    sprintf (s, "%c%s", bot->share, bot->bot);
				  if (showver)
				    sprintf (s + i, " (%d.%d.%d.%d)",
					     bot->ver / 1000000,
					     bot->ver % 1000000 / 10000,
...
dump_links (int z)
{
  ...
  if (!(bot_flags (dcc[z].user) & BOT_ISOLATE))
      ...
      for (i = 0; i < parties; i++)
	{
	  l =
	    simple_sprintf (x, "j %s %s %D %c%D %s\n", party[i].bot,
			    party[i].nick, party[i].chan, party[i].flag,
...
int
botunlink (int idx, char *nick, char *reason)
{
  char s[20];
  register int i;
  int bots, users;
      ...
      if ((nick[0] == '*') || !egg_strcasecmp (dcc[i].nick, nick))
	{
	  if (dcc[i].type == &DCC_FORK_BOT)
		...
		dprintf (idx, "%s: %s -> %s.\n", BOT_KILLLINKATTEMPT,
	      ...
	      putlog (LOG_BOTS, "*", "%s: %s -> %s:%d", BOT_KILLLINKATTEMPT,
		...
		dprintf (idx, "%s %s.\n", BOT_ENDLINKATTEMPT, dcc[i].nick);
	      putlog (LOG_BOTS, "*", "%s %s @ %s:%d",
		      "Stopped trying to link", dcc[i].nick, dcc[i].host,
		      dcc[i].port);
	      killsock (dcc[i].sock);
	      lostdcc (i);
	      if (nick[0] != '*')
		...
		{
		  simple_sprintf (s,
				  "%s %s (%s) (lost %d bot%s and %d user%s)",
	      ...
	      else
		{
		  simple_sprintf (s, "%s %s (lost %d bot%s and %d user%s)",
				  BOT_UNLINKEDFROM, dcc[i].nick, bots,
				  (bots != 1) ? "s" : "", users,
				  (users != 1) ? "s" : "");
  ...
  if (nick[0] != '*')
	...
	{
	  char *ghost =
	    "BUG!!: Found bot `%s' in internal bot list, but it\n"
...
int
botlink (char *linker, int idx, char *nick)
{
  struct bot_addr *bi;
  struct userrec *u;
  register int i;
...
failed_tandem_relay (int idx)
{
  ...
  dcc[idx].timeval = now;
  if (dcc[idx].sock < 0
      || open_telnet_raw (dcc[idx].sock,
			  dcc[idx].
			  addr ? iptostr (htonl (dcc[idx].addr)) : dcc[idx].
			  host, dcc[idx].port) < 0)
...
void
tandem_relay (int idx, char *nick, register int i)
{
  struct userrec *u;
  struct bot_addr *bi;
  struct chat_info *ci;
  ...
  bi = (struct bot_addr *) get_user (&USERENTRY_BOTADDR, u);
  if (!bi || !strlen (bi->address) || !bi->relay_port
      || (bi->relay_port <= 0))
    {
      dprintf (idx, "%s '%s'.\n", BOT_NOTELNETADDY, nick);
      dprintf (idx, "%s .chaddr %s %s\n", MISC_USEFORMAT, nick,
  ...
  dcc[i].sock =
    getsock (SOCK_STRONGCONN | SOCK_VIRTUAL, getprotocol (bi->address));
  if (dcc[i].sock < 0)
    {
      lostdcc (i);
      dprintf (idx, "%s\n", MISC_NOFREESOCK);
  ...
  strcpy (dcc[i].host, bi->address);
#ifdef HUB
  dprintf (idx, "%s %s @ %s:%d ...\n", BOT_CONNECTINGTO, nick, bi->address,
	   bi->relay_port);
#endif
  dprintf (idx, "%s\n", BOT_BYEINFO1);
...
tandem_relay_resolve_failure (int idx)
{
  struct chat_info *ci;
...
static void
tandem_relay_resolve_success (int i)
{
  ...
  dcc[i].timeval = now;
  if (open_telnet_raw
      (dcc[i].sock, iptostr (htonl (dcc[i].addr)), dcc[i].port) < 0)
    failed_tandem_relay (i);
}
static void
pre_relay (int idx, char *buf, register int i)
{
  register int tidx = (-1);
  for (i = 0; i < dcc_total; i++)
...
static void
cont_tandem_relay (int idx, char *buf, register int i)
{
  register int uidx = (-1);
  ...
  dprintf (uidx, "%s\n\n", BOT_BYEINFO2);
  putlog (LOG_MISC, "*", "%s %s -> %s", BOT_RELAYLINK, dcc[uidx].nick,
	  dcc[idx].nick);
  ri = dcc[uidx].u.relay;
  dcc[uidx].type = &DCC_CHAT;
...
static void
dcc_relay (int idx, char *buf, int j)
{
  unsigned char *p = (unsigned char *) buf;
  int mark;
  for (j = 0;
       dcc[j].sock != dcc[idx].u.relay->sock || dcc[j].type != &DCC_RELAYING;
       j++);
  if (!(dcc[j].status & STAT_TELNET))
    {
      while (*p != 0)
...
static void
dcc_relaying (int idx, char *buf, int j)
{
  struct chat_info *ci;
  if (egg_strcasecmp (buf, "*bye*"))
    {
  ...
  for (j = 0;
       (dcc[j].sock != dcc[idx].u.relay->sock) || (dcc[j].type != &DCC_RELAY);
       j++);
  dcc[idx].status = dcc[idx].u.relay->old_status;
  ...
  dprintf (idx, "%s %s.\n\n", BOT_ABORTRELAY2, botnetnick);
  putlog (LOG_MISC, "*", "%s: %s -> %s", BOT_RELAYBROKEN, dcc[idx].nick,
	  dcc[j].nick);
...
static void
display_relay (int i, char *other)
{
  sprintf (other, "rela  -> sock %d", dcc[i].u.relay->sock);
} static void
display_relaying (int i, char *other)
{
  sprintf (other, ">rly  -> sock %d", dcc[i].u.relay->sock);
} static void
display_tandem_relay (int i, char *other)
{
  strcpy (other, "other  rela");
} static void
display_pre_relay (int i, char *other)
{
...
struct dcc_table DCC_RELAY =
  { "RELAY", 0, eof_dcc_relay, dcc_relay, NULL, NULL, display_relay,
expmem_relay, kill_relay, NULL };
static void
out_relay (int idx, char *buf, void *x)
{
  register struct relay_info *p = (struct relay_info *) x;
  if (p && p->chat)
...
struct dcc_table DCC_RELAYING =
  { "RELAYING", 0, eof_dcc_relaying, dcc_relaying, NULL, NULL,
display_relaying, expmem_relay, kill_relay, out_relay };
struct dcc_table DCC_FORK_RELAY =
  { "FORK_RELAY", 0, failed_tandem_relay, cont_tandem_relay, &connect_timeout,
failed_tandem_relay, display_tandem_relay, expmem_relay, kill_relay, NULL };
struct dcc_table DCC_PRE_RELAY =
  { "PRE_RELAY", 0, failed_pre_relay, pre_relay, NULL, NULL,
display_pre_relay, expmem_relay, kill_relay, NULL };
void
check_botnet_pings ()
{
  int i;
		...
		if (dcc[i].status & STAT_WARNED)
		  {
		    ...
		    simple_sprintf (s,
				    "%s %s (%s) (lost %d bot%s and %d user%s)",
				    BOT_DISCONNECTED, dcc[i].nick,
				    BOT_BOTNOTLEAFLIKE, bots,
				    (bots != 1) ? "s" : "", users,
				    (users != 1) ? "s" : "");
		    chatout ("*** %s\n", s);
		    botnet_send_unlinked (i, dcc[i].nick, s);
		  ...
		  {
		    putlog (LOG_MISC, "*",
			    "I am lame, and am now rejecting %s", bot->bot);
		    botnet_send_reject (i, botnetnick, NULL, bot->bot, NULL,
					NULL);
