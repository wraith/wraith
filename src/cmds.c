...
extern int dcc_total, remote_boots, backgrd, do_restart, conmask,
  must_be_owner, strict_host, quiet_save;
extern unsigned long otraffic_irc, otraffic_irc_today, itraffic_irc,
  ...
  itraffic_trans_today, otraffic_unknown, otraffic_unknown_today,
...
tell_who (struct userrec *u, int idx, int chan)
{
  ...
  for (i = 0; i < dcc_total; i++)
      ...
      if (dcc[i].u.chat->channel == chan)
	{
	  if (atr & USER_OWNER)
	    ...
	    {
	      egg_snprintf (format, sizeof format, "  %%c%%-%us %%s",
			    nicklen);
	      sprintf (s, format, (geticon (i) == '-' ? ' ' : geticon (i)),
  ...
  for (i = 0; i < dcc_total; i++)
      ...
      if ((dcc[i].type == &DCC_CHAT) && (dcc[i].u.chat->channel != chan))
	{
	  ...
	  if (atr & USER_OWNER)
	    {
	      egg_snprintf (format, sizeof format, "  [%%.2lu]  %%c%%-%us ",
			    nicklen);
	      sprintf (s, format, dcc[i].sock,
		       (geticon (i) == '-' ? ' ' : geticon (i)), dcc[i].nick);
	    }
	  ...
	  && (dcc[i].type != &DCC_CHAT))
	{
	  ...
	  if (atr & USER_OWNER)
	    {
	      egg_snprintf (format, sizeof format,
			    "  [%%.2lu]  %%c%%-%us (files) %%s", nicklen);
	      sprintf (s, format, dcc[i].sock,
		       dcc[i].status & STAT_CHAT ? '+' : ' ', dcc[i].nick,
		       dcc[i].host);
	    }
	  else
	    {
	      egg_snprintf (format, sizeof format, "  %%c%%-%us (files) %%s",
			    nicklen);
	      sprintf (s, format, dcc[i].status & STAT_CHAT ? '+' : ' ',
		       dcc[i].nick, dcc[i].host);
	    }
	  dprintf (idx, "%s\n", s);
	}
...
static void
cmd_botinfo (struct userrec *u, int idx, char *par)
{
  char s[512], s2[32];
  struct chanset_t *chan;
  time_t now2;
...
#endif
static void
cmd_whom (struct userrec *u, int idx, char *par)
{
  if (par[0] == '*')
    {
...
void
cmd_botconfig (struct userrec *u, int idx, char *par)
{
  struct userrec *u2;
  char *p;
  struct xtra_key *k;
  ...
  cfgent = NULL;
  for (i = 0; !cfgent && (i < cfg_count); i++)
    if (!strcmp (cfg[i]->name, p) && (cfg[i]->flags & CFGF_LOCAL)
	&& (cfg[i]->describe))
      cfgent = cfg[i];
  if (!cfgent)
    {
  ...
  if (par[0])
    {
      set_cfg_str (u2->handle, cfgent->name,
		   (strcmp (par, "-")) ? par : NULL);
      dprintf (idx, STR ("Now: "));
#ifdef HUB
      write_userfile (idx);
  ...
  if (!cmd[0] || par[0])
    {
      ...
      dprintf (idx,
	       STR
	       ("  if no password is specified, the commands password is reset\n"));
      return;
    }
  for (i = 0; cmd[i]; i++)
    cmd[i] = tolower (cmd[i]);
...
#endif
static void
cmd_lagged (struct userrec *u, int idx, char *par)
{
  int i;
  ...
  for (i = 0; i < dcc_total; i++)
      ...
      if (dcc[i].type == &DCC_BOT)
	{
	  dprintf (idx, STR ("%9s - %i seconds\n"), dcc[i].nick,
		   (dcc[i].pingtime >
		    120) ? (now - dcc[i].pingtime) : dcc[i].pingtime);
	}
    }
}
#endif
static void
cmd_me (struct userrec *u, int idx, char *par)
{
  int i;
  ...
  for (i = 0; i < dcc_total; i++)
	...
	&& (dcc[i].u.chat->channel == dcc[idx].u.chat->channel) && ((i != idx)
								    ||
								    (dcc[i].
								     status &
								     STAT_ECHO)))
      dprintf (i, "* %s %s\n", dcc[idx].nick, par);
  botnet_send_act (idx, botnetnick, dcc[idx].nick, dcc[idx].u.chat->channel,
		   par);
  check_tcl_act (dcc[idx].nick, dcc[idx].u.chat->channel, par);
}

void
cmd_motd (struct userrec *u, int idx, char *par)
{
...
static void
cmd_back (struct userrec *u, int idx, char *par)
{
  not_away (idx);
} static void
cmd_newpass (struct userrec *u, int idx, char *par)
{
  char *new;
...
#ifdef HUB
static void
cmd_bots (struct userrec *u, int idx, char *par)
{
  putlog (LOG_CMDS, "*", "#%s# bots", dcc[idx].nick);
  tell_bots (idx);
} static void
cmd_downbots (struct userrec *u, int idx, char *par)
{
  struct userrec *u2;
  int cnt = 0;
  char work[128] = "";
...
static void
cmd_bottree (struct userrec *u, int idx, char *par)
{
  putlog (LOG_CMDS, "*", "#%s# bottree", dcc[idx].nick);
  tell_bottree (idx, 0);
} static void
cmd_vbottree (struct userrec *u, int idx, char *par)
{
...
#endif
static int
my_cmp (const mycmds * c1, const mycmds * c2)
{
  return strcmp (c1->name, c2->name);
}
static void
cmd_help (struct userrec *u, int idx, char *par)
{
  char flg[100];
  ...
  for (o = 0; o < cmdi; o++)
      ...
      if (!showall && !strcasecmp (par, cmds[o].name))
	{
	  ...
	  dprintf (idx, STR ("Usage      : %s%s %s\n"), dcc_prefix,
		   cmds[o].name, cmds[o].usage ? cmds[o].usage : "");
...
#ifdef HUB
static void
cmd_addlog (struct userrec *u, int idx, char *par)
{
...
static void
cmd_who (struct userrec *u, int idx, char *par)
{
  int i;
  if (par[0])
    {
      ...
      else
	{
	  ...
	  if (i < 0)
	    ...
	    {
	      char s[40];
	      simple_sprintf (s, "%d:%s@%s", dcc[idx].sock, dcc[idx].nick,
			      botnetnick);
...
#endif
static void
cmd_whois (struct userrec *u, int idx, char *par)
{
  if (!par[0])
    {
...
static void
cmd_match (struct userrec *u, int idx, char *par)
{
...
static void
cmd_uptime (struct userrec *u, int idx, char *par)
{
  putlog (LOG_CMDS, "*", "#%s# uptime", dcc[idx].nick);
  tell_verbose_uptime (idx);
} static void
cmd_userlist (struct userrec *u, int idx, char *par)
{
  int cnt = 0;
...
static void
cmd_channels (struct userrec *u, int idx, char *par)
{
  struct chanset_t *chan;
  putlog (LOG_CMDS, "*", STR ("#%s# channels"), dcc[idx].nick);
...
static void
cmd_status (struct userrec *u, int idx, char *par)
{
  int atr = u ? u->flags : 0;
...
#ifdef HUB
static void
cmd_dccstat (struct userrec *u, int idx, char *par)
{
...
#endif
static void
cmd_boot (struct userrec *u, int idx, char *par)
{
  int i, files = 0, ok = 0;
  char *who;
  struct userrec *u2;
  ...
  if (strchr (who, '@') != NULL)
      ...
      if (remote_boots > 0)
	{
	  ...
	  putlog (LOG_BOTS, "*", "#%s# boot %s@%s (%s)", dcc[idx].nick,
...
static void
cmd_console (struct userrec *u, int idx, char *par)
{
	  ...
	  if (*nick == '+')
	      ...
	      if ((dest == idx) && !glob_master (fr) && pls)
		{
		  if (chan_master (fr))
		    ...
		    md &=
		      ~(LOG_MISC | LOG_CMDS | LOG_FILES | LOG_WALL |
			LOG_DEBUG);
		}
...
cmd_chhandle (struct userrec *u, int idx, char *par)
{
  ...
  if (strchr (BADHANDCHARS, newhand[0]) != NULL)
	...
	dprintf (idx, "You can't change a permanent bot owner's handle.\n");
      else if (!egg_strcasecmp (newhand, botnetnick)
	       && (!(atr2 & USER_BOT) || nextbot (hand) != -1))
	dprintf (idx, "Hey! That's MY name!\n");
      else if (change_handle (u2, newhand))
	{
	  putlog (LOG_CMDS, "*", "#%s# chhandle %s %s", dcc[idx].nick, hand,
		  newhand);
	  dprintf (idx, "Changed.\n");
	}
      else
...
#endif
static void
cmd_handle (struct userrec *u, int idx, char *par)
{
  ...
  for (i = 0; i < strlen (newhandle); i++)
    if ((newhandle[i] <= 32) || (newhandle[i] >= 127)
	|| (newhandle[i] == '@'))
      newhandle[i] = '?';
...
#ifdef HUB
static void
cmd_chpass (struct userrec *u, int idx, char *par)
{
  char *handle, *new;
...
cmd_uplink (struct userrec *u, int idx, char *par)
{
  char *handle, *uplink;
  struct bot_addr *bi, *obi;
  struct userrec *u1;
...
cmd_chaddr (struct userrec *u, int idx, char *par)
{
...
#endif
static void
cmd_comment (struct userrec *u, int idx, char *par)
{
  char *handle;
  struct userrec *u1;
  handle = newsplit (&par);
...
#ifdef HUB
static void
cmd_reload (struct userrec *u, int idx, char *par)
{
...
#endif
void
cmd_die (struct userrec *u, int idx, char *par)
{
  char s1[1024], s2[1024];
  putlog (LOG_CMDS, "*", "#%s# die %s", dcc[idx].nick, par);
  if (par[0])
    {
      egg_snprintf (s1, sizeof s1, "BOT SHUTDOWN (%s: %s)", dcc[idx].nick,
		    par);
...
static void
cmd_debug (struct userrec *u, int idx, char *par)
{
  putlog (LOG_CMDS, "*", "#%s# debug", dcc[idx].nick);
  debug_mem_to_dcc (idx);
} static void
cmd_simul (struct userrec *u, int idx, char *par)
{
  char *nick;
  int i, ok = 0;
  nick = newsplit (&par);
...
#ifdef HUB
static void
cmd_link (struct userrec *u, int idx, char *par)
{
  char *s;
  int i;
  if (!par[0])
  ...
  if (!par[0] || !egg_strcasecmp (par, botnetnick))
      ...
      if (i < 0)
	{
      ...
      simple_sprintf (x, "%d:%s@%s", dcc[idx].sock, dcc[idx].nick,
		      botnetnick);
...
#endif
static void
cmd_unlink (struct userrec *u, int idx, char *par)
{
  int i;
  char *bot;
  if (!par[0])
...
cmd_relay (struct userrec *u, int idx, char *par)
{
  if (!par[0])
    {
...
static void
cmd_binds (struct userrec *u, int idx, char *par)
{
...
#endif
int
check_dcc_attrs (struct userrec *u, int oatr)
{
  int i, stat, lostp = 0;
  if (!u)
    return 0;
  ...
  for (i = 0; i < dcc_total; i++)
	  ...
	  && (!egg_strcasecmp (u->handle, dcc[i].nick)))
	{
	  ...
	  if ((oatr & USER_MASTER) && !(u->flags & USER_MASTER))
		...
		~(LOG_MISC | LOG_CMDS | LOG_RAW | LOG_FILES | LOG_WALL |
		  LOG_DEBUG);
	  ...
	  if (!(oatr & USER_PARTY) && (u->flags & USER_PARTY))
	    {
	      dprintf (i, "-+- POOF! -+-\n");
	      dprintf (i, "You now have party line chat access.\n");
	      dprintf (i, "To rejoin the partyline, type: %schat 0\n",
		       dcc_prefix);
	    }
	  if ((oatr & USER_PARTY) && !(u->flags & USER_PARTY))
	    lostp = 1;
	  ...
	  if (!(u->flags & USER_PARTY) && (dcc[i].u.chat->channel != -1)
	      && lostp)
	    {
	      ...
	      chanout_but (-1, dcc[i].u.chat->channel,
			   "*** %s left the party line - no chat access.\n",
			   dcc[i].nick);
...
int
check_dcc_chanattrs (struct userrec *u, char *chname, int chflags, int ochatr)
{
...
static void
cmd_chattr (struct userrec *u, int idx, char *par)
{
  char *hand, *arg = NULL, *tmpchg = NULL, *chg = NULL, work[1024];
  struct chanset_t *chan = NULL;
  struct userrec *u2;
  struct flag_record pls = { 0, 0, 0, 0, 0, 0 }, mns =
  {
  0, 0, 0, 0, 0, 0}, user =
  {
  0, 0, 0, 0, 0, 0};
  ...
  if (chg)
      ...
      if (!isowner (u->handle))
	{
	  if (pls.global &USER_HUBA)
	    putlog (LOG_MISC, "*",
		    "%s attempted to give %s hub connect access",
		    dcc[idx].nick, u2->handle);
	  if (mns.global &USER_HUBA)
	    putlog (LOG_MISC, "*",
		    "%s attempted to take away hub connect access from %s",
		    dcc[idx].nick, u2->handle);
	  if (pls.global &USER_ADMIN)
	    putlog (LOG_MISC, "*", "%s attempted to give %s admin access",
		    dcc[idx].nick, u2->handle);
	  if (mns.global &USER_ADMIN)
	    putlog (LOG_MISC, "*",
		    "%s attempted to take away admin access from %s",
		    dcc[idx].nick, u2->handle);
	  if (pls.global &USER_OWNER)
	    putlog (LOG_MISC, "*", "%s attempted to give owner to %s",
		    dcc[idx].nick, u2->handle);
	  if (mns.global &USER_OWNER)
	    putlog (LOG_MISC, "*", "%s attempted to take owner away from %s",
		    dcc[idx].nick, u2->handle);
	...
	{
	  pls.
	    global &=~(USER_CHUBA | USER_OWNER | USER_MASTER | USER_UNSHARED);
	  mns.
	    global &=~(USER_CHUBA | USER_OWNER | USER_MASTER | USER_UNSHARED);
	  if (chan)
	    {
      ...
      if (user.match & FR_GLOBAL)
	{
	  ...
	  user.udef_global =
	    (user.udef_global | pls.udef_global) & ~mns.udef_global;
	}
      if (chan)
	{
	  ocf = user.chan;
	  user.chan =
	    chan_sanity_check ((user.chan | pls.chan) & ~mns.chan,
			       user.global);
  ...
  if (chan)
    putlog (LOG_CMDS, "*", "#%s# (%s) chattr %s %s", dcc[idx].nick,
	    chan ? chan->dname : "*", hand, chg ? chg : "");
  else
...
#ifdef HUB
static void
cmd_botattr (struct userrec *u, int idx, char *par)
{
  ...
  if (!glob_master (user))
    {
      dprintf (idx, "You do not have Master privileges.\n");
      if (tmpchg)
	nfree (tmpchg);
      return;
    }
  ...
  if (chan)
    putlog (LOG_CMDS, "*", "#%s# (%s) botattr %s %s", dcc[idx].nick,
	    chan->dname, hand, chg ? chg : "");
  else
...
#endif
static void
cmd_chat (struct userrec *u, int idx, char *par)
{
  char *arg;
  int newchan, oldchan;
  module_entry *me;
      ...
      if (arg[0] == '*')
	{
	  if (((arg[1] < '0') || (arg[1] > '9')))
	      ...
	      else
		{
		  ...
		  if ((Tcl_VarEval (interp, "assoc ", "$chan", NULL) ==
		       TCL_OK) && interp->result[0])
...
int
exec_str (struct userrec *u, int idx, char *cmd)
{
  char *out, *err, *p, *np;
...
static void
cmd_w (struct userrec *u, int idx, char *par)
{
...
static void
cmd_ps (struct userrec *u, int idx, char *par)
{
  char *buf;
  ...
  if (strchr (par, '|') || strchr (par, '<') || strchr (par, ';')
    ...
    {
      putlog (LOG_WARN, "*",
	      STR ("%s attempted 'ps' with pipe/semicolon in parameters: %s"),
	      dcc[idx].nick, par);
      dprintf (idx, "No.");
      return;
    }
...
static void
cmd_last (struct userrec *u, int idx, char *par)
{
  char user[20], buf[30];
  struct passwd *pw;
  ...
  if (strchr (par, '|') || strchr (par, '<') || strchr (par, ';')
      || strchr (par, '>'))
    {
      putlog (LOG_WARN, "*",
	      STR
	      ("%s attempted 'last' with pipe/semicolon in parameters: %s"),
	      dcc[idx].nick, par);
      dprintf (idx, "No.");
      return;
    }
...
static void
cmd_echo (struct userrec *u, int idx, char *par)
{
  module_entry *me;
  if (!par[0])
    {
...
static void
cmd_color (struct userrec *u, int idx, char *par)
{
  module_entry *me;
  char *type, *of;
...
static void
cmd_strip (struct userrec *u, int idx, char *par)
{
  ...
  if (dest == idx)
  ...
  else
    {
      ...
      dprintf (dest, "%s set your strip settings to: %s (%s).\n",
	       dcc[idx].nick, stripmasktype (dcc[dest].u.chat->strip_flags),
	       stripmaskname (dcc[dest].u.chat->strip_flags));
    }
...
static void
cmd_su (struct userrec *u, int idx, char *par)
{
  int atr = u ? u->flags : 0;
  int ok;
  ...
  if (!par[0])
    ...
    dprintf (idx,
	     "You can't su to a bot... then again, why would you wanna?\n");
      ...
      else
	{
	  ...
	  if (!(atr & USER_OWNER)
		...
		{
		  dprintf (idx,
			   "No password set for user. You may not .su to them.\n");
		  return;
		}
	      if (dcc[idx].u.chat->channel < GLOBAL_CHANS)
		botnet_send_part_idx (idx, "");
		...
		nfree (dcc[idx].u.chat->away);
	      dcc[idx].u.chat->away =
		get_data_ptr (strlen (dcc[idx].nick) + 1);
		...
		get_data_ptr (strlen (dcc[idx].nick) + 1);
	      ...
	      dprintf (idx, "Enter password for %s%s\n", par,
		       (dcc[idx].
			status & STAT_TELNET) ? TLN_IAC_C TLN_WILL_C
		       TLN_ECHO_C : "");
...
static void
cmd_fixcodes (struct userrec *u, int idx, char *par)
{
...
static void
cmd_page (struct userrec *u, int idx, char *par)
{
  int a;
  module_entry *me;
  if (!par[0])
  ...
  if (!par[0])
    {
      ...
      dprintf (idx,
	       STR
	       ("       Leafs can't link unless you specify a *!ident@ip hostmask\n"));
    }
  else
    {
      handle = newsplit (&par);
	...
	{
	  userlist =
	    adduser (userlist, handle, STR ("none"), "-",
		     USER_BOT | USER_FRIEND | USER_OP);
	  u1 = get_user_by_handle (userlist, handle);
	  bi = user_malloc (sizeof (struct bot_addr));
...
static void
cmd_set (struct userrec *u, int idx, char *msg)
{
  int code;
  char s[512];
  if (!(isowner (dcc[idx].nick)) && (must_be_owner))
...
static void
cmd_mns_ignore (struct userrec *u, int idx, char *par)
{
  char buf[UHOSTLEN];
  if (!par[0])
    {
...
static void
cmd_ignores (struct userrec *u, int idx, char *par)
{
  putlog (LOG_CMDS, "*", "#%s# ignores %s", dcc[idx].nick, par);
  tell_ignores (idx, par);
} static void
cmd_pls_user (struct userrec *u, int idx, char *par)
{
  char *handle, *host;
...
static void
cmd_mns_user (struct userrec *u, int idx, char *par)
{
  int idx2;
  char *handle;
  struct userrec *u2;
...
static void
cmd_pls_host (struct userrec *u, int idx, char *par)
{
  char *handle, *host;
  struct userrec *u2;
  struct list_type *q;
  struct flag_record fr2 =
    { FR_GLOBAL | FR_CHAN | FR_ANYWH, 0, 0, 0, 0, 0 }, fr =
  {
  ...
  if (egg_strcasecmp (handle, dcc[idx].nick))
	...
	{
	  dprintf (idx,
		   "You can't add hostmasks to a channel owner/master.\n");
	  return;
	}
...
static void
cmd_mns_host (struct userrec *u, int idx, char *par)
{
  char *handle, *host;
  struct userrec *u2;
  struct flag_record fr2 =
  ...
  if (glob_deop (fr) || glob_kick (fr) || chan_deop (fr) || chan_kick (fr))
    {
      dprintf (idx,
	       "You can't remove hostmasks while having the +d or +k "
	       "flag.\n");
      return;
    }
  if (egg_strcasecmp (handle, dcc[idx].nick))
	...
	{
	  dprintf (idx,
		   "You can't remove hostmasks from a bot owner/master.\n");
	  return;
	}
      if ((chan_owner (fr2) || chan_master (fr2)) && !glob_master (fr)
	  && !glob_owner (fr) && !chan_owner (fr))
	{
	  dprintf (idx,
		   "You can't remove hostmasks from a channel owner/master.\n");
	  return;
	}
      if (!glob_master (fr) && !chan_master (fr))
	{
...
void
rcmd_cursrv (char *fbot, char *fhand, char *fidx)
{
#ifdef LEAF
...
void
rcmd_ver (char *fbot, char *fhand, char *fidx)
{
  char tmp[2048];
#ifdef HAVE_UNAME
  struct utsname un;
  ...
  if (uname (&un) < 0)
    ...
    {
      sprintf (tmp + strlen (tmp), STR ("%s %s (%s)"), un.sysname, un.release,
	       un.machine);
    }
#endif
  botnet_send_cmdreply (botnetnick, fbot, fhand, fidx, tmp);
...
void
rcmd_curnick (char *fbot, char *fhand, char *fidx)
{
#ifdef LEAF
  char tmp[1024];
  int server_online = 0;
...
static void
cmd_botmsg (struct userrec *u, int idx, char *par)
{
  char *tnick, *tbot;
  char tmp[1024];
  ...
  if (!par[0])
    {
      dprintf (idx,
	       STR ("Usage: botmsg <botname> <nick|#channel> <message>\n"));
      return;
    }
  if (nextbot (tbot) < 0)
    {
...
static void
cmd_netmsg (struct userrec *u, int idx, char *par)
{
  char *tnick;
  char tmp[1024];
...
void
rcmd_msg (char *tobot, char *frombot, char *fromhand, char *fromidx,
	  char *par)
{
#ifdef LEAF
  char buf[1024], *nick;
...
static void
cmd_netlag (struct userrec *u, int idx, char *par)
{
  struct timeval tv;
  time_t tm;
  char tmp[64];
...
rcmd_ping (char *frombot, char *fromhand, char *fromidx, char *par)
{
  char tmp[64];
  sprintf (tmp, STR ("pong %s"), par);
  botnet_send_cmd (botnetnick, frombot, fromhand, atoi (fromidx), tmp);
} void
rcmd_pong (char *frombot, char *fromhand, char *fromidx, char *par)
{
  int i = atoi (fromidx);
  if ((i >= 0) && (i < dcc_total) && (dcc[i].type == &DCC_CHAT)
      && (!strcmp (dcc[i].nick, fromhand)))
    {
      struct timeval tv;
      time_t tm;
      gettimeofday (&tv, NULL);
      tm =
	((tv.tv_sec % 10000) * 100 + (tv.tv_usec * 100) / (1000000)) -
	atoi (par);
...
static void
cmd_netw (struct userrec *u, int idx, char *par)
{
  char tmp[128];
...
cmd_botps (struct userrec *u, int idx, char *par)
{
  ...
  if (strchr (par, '|') || strchr (par, '<') || strchr (par, ';')
      || strchr (par, '>'))
    {
      putlog (LOG_WARN, "*",
	      STR
	      ("%s attempted 'botps' with pipe/semicolon in parameters: %s"),
	      dcc[idx].nick, par);
      dprintf (idx, STR ("No."));
      return;
    }
...
cmd_netps (struct userrec *u, int idx, char *par)
{
  ...
  if (strchr (par, '|') || strchr (par, '<') || strchr (par, ';')
      || strchr (par, '>'))
    {
      putlog (LOG_WARN, "*",
	      STR
	      ("%s attempted 'netps' with pipe/semicolon in parameters: %s"),
	      dcc[idx].nick, par);
      dprintf (idx, STR ("No."));
      return;
    }
...
static void
cmd_botlast (struct userrec *u, int idx, char *par)
{
  char *tbot, buf[1024];
  ...
  if (strchr (par, '|') || strchr (par, '<') || strchr (par, ';')
      || strchr (par, '>'))
    {
      putlog (LOG_WARN, "*",
	      STR
	      ("%s attempted 'botlast' with pipe/semicolon in parameters: %s"),
	      dcc[idx].nick, par);
      dprintf (idx, STR ("No."));
      return;
    }
...
static void
cmd_netlast (struct userrec *u, int idx, char *par)
{
  char buf[1024];
  ...
  if (strchr (par, '|') || strchr (par, '<') || strchr (par, ';')
      || strchr (par, '>'))
    {
      putlog (LOG_WARN, "*",
	      STR
	      ("%s attempted 'netlast' with pipe/semicolon in parameters: %s"),
	      dcc[idx].nick, par);
      dprintf (idx, STR ("No."));
      return;
    }
  ...
  if (!strcmp (cmd, "w"))
      ...
      if (!user[0])
	{
	  botnet_send_cmdreply (botnetnick, frombot, fromhand, fromidx,
				STR ("Can't determine user id for process"));
	  return;
	}
...
cmd_botjump (struct userrec *u, int idx, char *par)
{
  ...
  if (!tbot[0])
    {
      dprintf (idx,
	       STR ("Usage: botjump <botname> [server [port [pass]]]\n"));
      return;
    }
  if (nextbot (tbot) < 0)
    {
...
void
rcmd_jump (char *frombot, char *fromhand, char *fromidx, char *par)
{
#ifdef LEAF
  char *other;
  module_entry *me;
  ...
  if (par[0])
    {
      ...
      if (!port)
	port = default_port;
      strncpy0 (((char *) (func[20])), other, 120);
      (*(int *) (func[21])) = port;
      strncpy0 (((char *) (func[22])), par, 120);
    }
  botnet_send_cmdreply (botnetnick, frombot, fromhand, fromidx,
			STR ("Jumping..."));
  (*(int *) (func[23])) = 0;
...
gotremotecmd (char *forbot, char *frombot, char *fromhand, char *fromidx,
	      char *cmd)
{
  char *par = cmd;
  cmd = newsplit (&par);
  if (!strcmp (cmd, STR ("exec")))
    ...
    {
      botnet_send_cmdreply (botnetnick, frombot, fromhand, fromidx,
			    STR ("Unrecognized remote command"));
    }
}
void
gotremotereply (char *frombot, char *tohand, char *toidx, char *ln)
{
  int idx = atoi (toidx);
  if ((idx >= 0) && (idx < dcc_total) && (dcc[idx].type == &DCC_CHAT)
      && (!strcmp (dcc[idx].nick, tohand)))
    {
      dprintf (idx, STR ("(%s) %s\n"), frombot, ln);
    }
}
static void
cmd_traffic (struct userrec *u, int idx, char *par)
{
  ...
  if (otraffic_trans > 0 || itraffic_trans > 0 || otraffic_trans_today > 0
      || itraffic_trans_today > 0)
    {
      ...
      dprintf (idx, "  out: %s",
	       btos (otraffic_trans + otraffic_trans_today));
      dprintf (idx, " (%s today)\n", btos (otraffic_trans_today));
      dprintf (idx, "   in: %s",
	       btos (itraffic_trans + itraffic_trans_today));
...
static void
cmd_whoami (struct userrec *u, int idx, char *par)
{
