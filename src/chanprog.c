...
extern char ver[], botnetnick[], firewall[], myip[], origbotname[],
  motdfile[], userfile[], helpdir[], tempdir[], notify_new[], owner[],
  configfile[], netpass[], botuser[], owners[], hubs[];
extern time_t now, online_since;
extern int backgrd, term_z, con_chan, cache_hit, cache_miss, firewallport,
  default_flags, max_logs, conmask, protect_readonly, noshare,
#ifdef HUB
  my_port,
#endif
  ignore_time, loading;
...
memberlist *
ismember (struct chanset_t *chan, char *nick)
{
...
struct userrec *
check_chanlist (const char *host)
{
...
struct userrec *
check_chanlist_hand (const char *hand)
{
  register struct chanset_t *chan;
  register memberlist *m;
  for (chan = chanset; chan; chan = chan->next)
...
void
clear_chanlist_member (const char *nick)
{
  register memberlist *m;
  register struct chanset_t *chan;
  for (chan = chanset; chan; chan = chan->next)
...
void
set_chanlist (const char *host, struct userrec *rec)
{
  char *nick, *uhost, buf[UHOSTLEN];
  register memberlist *m;
  register struct chanset_t *chan;
  ...
  dprintf (idx, "%s %s (%s %s)\n", MISC_TCLVERSION,
	   ((interp)
		...
		TCL_OK)) ? interp->result : (Tcl_Eval (interp,
						       "info tclversion") ==
					     TCL_OK) ? interp->
	   result : "*unknown*", MISC_TCLHVERSION,
...
tell_settings (int idx)
{
  ...
  for (i = 0; i < max_logs; i++)
	...
	dprintf (idx, "Logfile #%d: %s on %s (%s: %s)\n", i + 1,
		 logs[i].filename, logs[i].chname, masktype (logs[i].mask),
		 maskname (logs[i].mask));
      }
...
load_internal_users ()
{
  char *p = NULL, *ln, *hand, *ip, *port, *hublevel =
    NULL, *pass, *hosts, host[250], buf[2048];
  char *attr;
  int i;
  struct bot_addr *bi;
  struct userrec *u;
	...
	*p++ = 0;
      ...
      for (i = 0; ln; i++)
	{
	  switch (i)
		...
		{
		  userlist =
		    adduser (userlist, hand, "none", "-",
			     USER_BOT | USER_OP | USER_FRIEND);
		  ...
		  bi->uplink[0] = 0;
		  set_user (&USERENTRY_BOTADDR,
			    get_user_by_handle (userlist, hand), bi);
		  set_user (&USERENTRY_PASS,
			    get_user_by_handle (userlist, hand), netpass);
		}
	    default:
	      hosts = ln;
	      ln = strchr (ln, ' ');
		...
		*ln++ = 0;
	      while (hosts)
		{
		  ...
		  set_user (&USERENTRY_HOSTS,
			    get_user_by_handle (userlist, hand), host);
		  hosts = ln;
		  if (ln)
		    ln = strchr (ln, ' ');
		  if (ln)
      ...
      for (i = 0; ln; i++)
	{
	  switch (i)
	      ...
	      if (!get_user_by_handle (userlist, hand))
		{
		  userlist =
		    adduser (userlist, hand, "none", "-",
			     USER_ADMIN | USER_OWNER | USER_MASTER |
			     USER_FRIEND | USER_OP | USER_PARTY | USER_HUBA |
			     USER_CHUBA);
...
chanprog ()
{
  ...
  if (!(u = get_user_by_handle (userlist, botnetnick)))
    {
      userlist =
	adduser (userlist, botnetnick, STR ("none"), "-",
		 USER_BOT | USER_OP | USER_FRIEND);
  ...
  bi =
    get_user (&USERENTRY_BOTADDR, get_user_by_handle (userlist, botnetnick));
  if (!bi)
...
add_timer (tcl_timer_t ** stack, int elapse, char *cmd, unsigned long prev_id)
{
...
int
remove_timer (tcl_timer_t ** stack, unsigned long id)
{
  tcl_timer_t *old;
  int ok = 0;
  while (*stack)
...
void
do_check_timers (tcl_timer_t ** stack)
{
...
void
wipe_timers (Tcl_Interp * irp, tcl_timer_t ** stack)
{
...
void
list_timers (Tcl_Interp * irp, tcl_timer_t * stack)
{
  tcl_timer_t *mark;
...
void
check_topic (struct chanset_t *chan)
{
  memberlist *m = NULL;
