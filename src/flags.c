#include "main.h"
extern int use_console_r, debug_output, noshare, allow_dk_cmds;
...
int
sanity_check (int atr)
{
  if ((atr & USER_BOT)
      && (atr &
	  (USER_PARTY | USER_MASTER | USER_OWNER | USER_ADMIN | USER_HUBA |
	   USER_CHUBA)))
    atr &=
      ~(USER_PARTY | USER_MASTER | USER_OWNER | USER_ADMIN | USER_HUBA |
	USER_CHUBA);
  if (!(atr & USER_BOT)
      && (atr &
	  (USER_DOLIMIT | USER_DOVOICE | USER_UPDATEHUB | USER_CHANHUB |
	   USER_SECHUB)))
    atr &=
      ~(USER_DOLIMIT | USER_DOVOICE | USER_UPDATEHUB | USER_CHANHUB |
	USER_SECHUB);
...
int
chan_sanity_check (int chatr, int atr)
{
...
void
break_down_flags (const char *string, struct flag_record *plus,
...
static int
flag2str (char *string, int bot, int udef)
{
  char x = 'a', *old = string;
  while (bot && (x <= 'z'))
    {
...
int
build_flags (char *string, struct flag_record *plus,
	     struct flag_record *minus)
{
  char *old = string;
...
void
set_user_flagrec (struct userrec *u, struct flag_record *fr,
		  const char *chname)
{
...
void
get_user_flagrec (struct userrec *u, struct flag_record *fr,
		  const char *chname)
{
  struct chanuserrec *cr = NULL;
  if (!u)
...
static int
botfl_unpack (struct userrec *u, struct user_entry *e)
{
...
static int
botfl_pack (struct userrec *u, struct user_entry *e)
{
  char x[100];
  struct flag_record fr = { FR_BOT, 0, 0, 0, 0, 0 };
  fr.bot = e->u.ulong;
...
static int
botfl_kill (struct user_entry *e)
{
  nfree (e);
  return 1;
}
static int
botfl_write_userfile (FILE * f, struct userrec *u, struct user_entry *e)
{
  char x[100];
  struct flag_record fr = { FR_BOT, 0, 0, 0, 0, 0 };
  fr.bot = e->u.ulong;
...
static int
botfl_set (struct userrec *u, struct user_entry *e, void *buf)
{
...
static int
botfl_tcl_get (Tcl_Interp * interp, struct userrec *u, struct user_entry *e,
	       int argc, char **argv)
{
  char x[100];
  struct flag_record fr = { FR_BOT, 0, 0, 0, 0, 0 };
...
static int
botfl_tcl_set (Tcl_Interp * irp, struct userrec *u, struct user_entry *e,
	       int argc, char **argv)
{
  struct flag_record fr = { FR_BOT, 0, 0, 0, 0, 0 };
...
static int
botfl_expmem (struct user_entry *e)
{
  return 0;
}
static void
botfl_display (int idx, struct user_entry *e)
{
  struct flag_record fr = { FR_BOT, 0, 0, 0, 0, 0 };
  char x[100];
  fr.bot = e->u.ulong;
  ...
  { 0, 0, def_dupuser, botfl_unpack, botfl_pack, botfl_write_userfile,
botfl_kill, def_get, botfl_set, botfl_tcl_get, botfl_tcl_set, botfl_expmem, botfl_display, "BOTFL" };
