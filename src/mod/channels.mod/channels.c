...
#include "udefchan.c"
static void
got_cset (char *botnick, char *code, char *par)
{
  int all = 0;
...
static void
got_cpart (char *botnick, char *code, char *par)
{
  char *chname;
...
static void
got_cycle (char *botnick, char *code, char *par)
{
  char *chname;
  struct chanset_t *chan;
  if (!par[0])
...
static void
got_down (char *botnick, char *code, char *par)
{
  char *chname;
  struct chanset_t *chan;
  if (!par[0])
...
static void
got_cjoin (char *botnick, char *code, char *par)
{
  char *chname;
  struct chanset_t *chan;
  if (!par[0])
  ...
  if (tcl_channel_add (0, chname, par) == TCL_ERROR)
    putlog (LOG_BOTS, "@",
	    "Invalid channel or channel options from %s for %s", botnick,
	    chname);
  else
    {
#ifdef HUB
      ...
      for (u = userlist; u; u = u->next)
	{
	  if ((u->flags & USER_BOT) && (nextbot (u->handle) >= 0))
	      ...
	      if (ba && (ba->roleid == (hNdx + 1)))
		{
		  ...
		  botnet_send_zapf (nextbot (u->handle), botnetnick,
				    u->handle, tmp);
		}
	    }
	}
...
static void
got_sj (int idx, char *code, char *par)
{
  char *chname;
  time_t delay;
  struct chanset_t *chan;
...
static void *
channel_malloc (int size, char *file, int line)
{
  char *p;
#ifdef DEBUG_MEM
...
static void
set_mode_protect (struct chanset_t *chan, char *set)
{
  int i, pos = 1;
  char *s, *s1;
...
static void
get_mode_protect (struct chanset_t *chan, char *s)
{
  char *p = s, s1[121];
  int ok = 0, i, tst;
  s1[0] = 0;
...
static int
ismodeline (masklist * m, char *user)
{
...
inline static int
chanset_unlink (struct chanset_t *chan)
{
...
static void
remove_channel (struct chanset_t *chan)
{
  int i;
  module_entry *me;
...
static int
channels_chon (char *handle, int idx)
{
...
static char *
convert_element (char *src, char *dst)
{
  int flags;
...
static cmd_t my_chon[] =
  { {"*", "", (Function) channels_chon, "channels:chon"}, {NULL, NULL, NULL,
							   NULL} };
static void
channels_report (int idx, int details)
{
  struct chanset_t *chan;
  int i;
  char s[1024], s2[100];
	  ...
	  if (!channel_inactive (chan))
	      ...
	      if (channel_active (chan))
		{
		  if (chan->dname[0] == '!')
		    {
		      dprintf (idx,
			       "    %-10s: %2d member%s enforcing \"%s\" (%s), "
			       "unique name %s\n", chan->dname,
			       chan->channel.members,
			       (chan->channel.members == 1) ? "," : "s,", s2,
			       s, chan->name);
		    }
		  else
		    {
...
static char *
traced_globchanset (ClientData cdata, Tcl_Interp * irp, CONST char *name1,
		    CONST char *name2, int flags)
#else
static char *
traced_globchanset (ClientData cdata, Tcl_Interp * irp, char *name1,
...
cmd_t channels_bot[] = { {"cjoin", "", (Function) got_cjoin, NULL}
...
static tcl_ints my_tcl_ints[] =
  { {"share-greet", NULL, 0}, {"use-info", &use_info, 0}, {"quiet-save",
							   &quiet_save, 0},
  {"global-stopnethack-mode", &global_stopnethack_mode, 0},
  {"global-revenge-mode", &global_revenge_mode, 0}, {"global-idle-kick",
						     &global_idle_kick, 0},
  {"global-ban-time", &global_ban_time, 0},
#ifdef S_IRCNET
{"global-exempt-time", &global_exempt_time, 0}, {"global-invite-time",
						 &global_invite_time, 0},
#endif
{"ban-time", &global_ban_time, 0},
#ifdef S_IRCNET
{"exempt-time", &global_exempt_time, 0}, {"invite-time", &global_invite_time,
					  0},
#endif
{NULL, NULL, 0}
};
static tcl_coups mychan_tcl_coups[] =
  ...
  {"global-flood-nick", &gfld_nick_thr, &gfld_nick_time}, {NULL, NULL,
							   NULL} };
static tcl_strings my_tcl_strings[] =
  { {"chanfile", chanfile, 120, STR_PROTECT}, {"global-chanmode",
					       glob_chanmode, 64, 0}, {NULL,
								       NULL,
								       0,
								       0} };
static char *
...
(Function) channels_expmem, (Function) channels_report, (Function) u_setsticky_mask, (Function) u_delban,
(Function) u_addban, (Function) write_bans, (Function) get_chanrec, (Function) add_chanrec,
(Function) del_chanrec, (Function) set_handle_chaninfo, (Function) channel_malloc, (Function) u_match_mask,
(Function) u_equals_mask, (Function) clear_channel, (Function) set_handle_laston, (Function) NULL,
(Function) & use_info, (Function) get_handle_chaninfo, (Function) u_sticky_mask, (Function) ismasked,
(Function) add_chanrec_by_handle, (Function) NULL, (Function) NULL, (Function) NULL, (Function) NULL,
(Function) NULL, (Function) NULL,
#ifdef S_IRCNET
  (Function) u_delexempt, (Function) u_addexempt,
#else
  (Function) NULL, (Function) NULL,
#endif
  (Function) NULL, (Function) NULL, (Function) NULL, (Function) NULL,
#ifdef S_IRCNET
  (Function) u_delinvite, (Function) u_addinvite,
#else
  (Function) NULL, (Function) NULL,
#endif
  (Function) tcl_channel_add, (Function) tcl_channel_modify,
#ifdef S_IRCNET
  (Function) write_exempts, (Function) write_invites,
#else
  (Function) NULL, (Function) NULL,
#endif
  (Function) ismodeline, (Function) initudef, (Function) ngetudef,
    (Function) expired_mask, (Function) remove_channel,
    (Function) & global_ban_time,
#ifdef S_IRCNET
  (Function) & global_exempt_time, (Function) & global_invite_time,
#else
  (Function) NULL, (Function) NULL,
#endif
  (Function) write_chans, (Function) write_config,
};
char *
channels_start (Function * global_funcs)
{
  global = global_funcs;
  gfld_chan_thr = 0;
  gfld_chan_time = 0;
