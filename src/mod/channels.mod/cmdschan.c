...
static void
cmd_pls_ban (struct userrec *u, int idx, char *par)
{
  ...
  if (!par[0])
    {
      dprintf (idx,
	       "Usage: +ban <hostmask> [channel] [%%<XdXhXm>] [reason]\n");
    }
  else
    {
      who = newsplit (&par);
	...
	egg_snprintf (s1, sizeof s1, "%s!%s@%s", origbotname, botuser,
		      hostname);
      if (wild_match (s, s1))
	{
	  ...
	  if (chan)
	      ...
	      if (par[0] == '*')
		{
		  ...
		  putlog (LOG_CMDS, "*", "#%s# (%s) +ban %s %s (%s) (sticky)",
			  dcc[idx].nick, dcc[idx].u.chat->con_chan, s,
			  chan->dname, par);
	      ...
	      if (par[0] == '*')
		{
		  ...
		  par++;
		  putlog (LOG_CMDS, "*",
			  "#%s# (GLOBAL) +ban %s (%s) (sticky)",
			  dcc[idx].nick, s, par);
		  dprintf (idx, "New sticky ban: %s (%s)\n", s, par);
		}
	      else
		{
		  putlog (LOG_CMDS, "*", "#%s# (GLOBAL) +ban %s (%s)",
			  dcc[idx].nick, s, par);
...
#ifdef S_IRCNET
static void
cmd_pls_exempt (struct userrec *u, int idx, char *par)
{
  char *chname, *who, s[UHOSTLEN], s1[UHOSTLEN], *p, *p_expire;
  unsigned long int expire_time = 0, expire_foo;
  struct chanset_t *chan = NULL;
  ...
  if (!par[0])
    {
      dprintf (idx,
	       "Usage: +exempt <hostmask> [channel] [%%<XdXhXm>] [reason]\n");
    }
  else
    {
      who = newsplit (&par);
      ...
      if (chan)
	{
	  ...
	  if (par[0] == '*')
	    ...
	    {
	      putlog (LOG_CMDS, "*", "#%s# (%s) +exempt %s %s (%s)",
		      dcc[idx].nick, dcc[idx].u.chat->con_chan, s,
		      chan->dname, par);
      ...
      else
	{
	  ...
	  if (par[0] == '*')
	    ...
	    {
	      putlog (LOG_CMDS, "*", "#%s# (GLOBAL) +exempt %s (%s)",
		      dcc[idx].nick, s, par);
...
static void
cmd_pls_invite (struct userrec *u, int idx, char *par)
{
  char *chname, *who, s[UHOSTLEN], s1[UHOSTLEN], *p, *p_expire;
  unsigned long int expire_time = 0, expire_foo;
  struct chanset_t *chan = NULL;
  ...
  if (!use_invites)
    {
      dprintf (idx,
	       "This command can only be used with use-invites enabled.\n");
      return;
    }
  if (!par[0])
    {
      dprintf (idx,
	       "Usage: +invite <hostmask> [channel] [%%<XdXhXm>] [reason]\n");
    }
  else
    {
      who = newsplit (&par);
      ...
      if (chan)
	{
	  ...
	  if (par[0] == '*')
	    ...
	    {
	      putlog (LOG_CMDS, "*", "#%s# (%s) +invite %s %s (%s)",
		      dcc[idx].nick, dcc[idx].u.chat->con_chan, s,
		      chan->dname, par);
      ...
      else
	{
	  ...
	  if (par[0] == '*')
	    ...
	    {
	      putlog (LOG_CMDS, "*", "#%s# (GLOBAL) +invite %s (%s)",
		      dcc[idx].nick, s, par);
...
#endif
static void
cmd_mns_ban (struct userrec *u, int idx, char *par)
{
  int console = 0, i = 0, j;
  struct chanset_t *chan = NULL;
...
#ifdef S_IRCNET
static void
cmd_mns_exempt (struct userrec *u, int idx, char *par)
{
  int i = 0, j;
  struct chanset_t *chan = NULL;
  ...
  if (chan)
      ...
      if (atoi (exempt) > 0)
	{
	  ...
	  i = 0;
	  for (e = chan->channel.exempt; e && e->mask && e->mask[0];
	       e = e->next)
	    {
		  ...
		  if (i == -j)
		    {
		      ...
		      dprintf (idx, "%s '%s' on %s.\n", IRC_REMOVEDEXEMPT,
			       e->mask, chan->dname);
		      putlog (LOG_CMDS, "*",
			      "#%s# (%s) -exempt %s [on channel]",
			      dcc[idx].nick, dcc[idx].u.chat->con_chan,
			      exempt);
		      return;
		    }
		}
...
static void
cmd_mns_invite (struct userrec *u, int idx, char *par)
{
  int i = 0, j;
  struct chanset_t *chan = NULL;
...
#endif
static void
cmd_bans (struct userrec *u, int idx, char *par)
{
...
#ifdef S_IRCNET
static void
cmd_exempts (struct userrec *u, int idx, char *par)
{
  if (!use_exempts)
    {
      dprintf (idx,
...
static void
cmd_invites (struct userrec *u, int idx, char *par)
{
  if (!use_invites)
    {
      dprintf (idx,
...
#endif
static void
cmd_info (struct userrec *u, int idx, char *par)
{
  ...
  if (!par[0])
	...
	dprintf (idx, "No info has been set for you.\n");
      putlog (LOG_CMDS, "*", "#%s# info %s", dcc[idx].nick,
	      chname ? chname : "");
      return;
    }
...
static void
cmd_chinfo (struct userrec *u, int idx, char *par)
{
  char *handle, *chname;
  struct userrec *u1;
  if (!use_info)
...
static void
cmd_slowjoin (struct userrec *u, int idx, char *par)
{
  ...
  if (!chname[0] || !p[0])
    {
      dprintf (idx,
	       "Usage: slowjoin channel interval-seconds [channel options]\n");
      return;
    }
  if (intvl < 10)
    {
      ...
      if ((bot->flags & USER_BOT) && ((nextbot (bot->handle) >= 0)))
	{
	  ...
	  if (!(bot_flags (bot) & BOT_LEAF))
	  ...
	  else
	    {
	      ...
	      sprintf (tmp, "sj %s %i\n", chan->dname, delay + v);
	      count++;
	    } botnet_send_zapf (nextbot (bot->handle), botnetnick,
				bot->handle, tmp);
	}
...
cmd_stick_yn (int idx, char *par, int yn)
{
  int i = 0, j;
...
#ifdef S_IRCNET
      dprintf (idx,
	       "Usage: %sstick [ban/exempt/invite] <hostmask or number> [channel]\n",
#else
  ...
  if (!egg_strcasecmp (stick_type, "exempt"))
	...
	{
	  i =
	    u_setsticky_exempt (NULL, s,
				(dcc[idx].user->
				 flags & USER_MASTER) ? yn : -1);
	  if (i > 0)
	    {
...
static void
cmd_stick (struct userrec *u, int idx, char *par)
{
  cmd_stick_yn (idx, par, 1);
} static void
cmd_unstick (struct userrec *u, int idx, char *par)
{
  cmd_stick_yn (idx, par, 0);
} static void
cmd_pls_chrec (struct userrec *u, int idx, char *par)
{
  char *nick, *chn;
  struct chanset_t *chan;
  struct userrec *u1;
  ...
  putlog (LOG_CMDS, "*", "#%s# +chrec %s %s", dcc[idx].nick, nick,
	  chan->dname);
  add_chanrec (u1, chan->dname);
  dprintf (idx, "Added %s channel record for %s.\n", chan->dname, nick);
}
static void
cmd_mns_chrec (struct userrec *u, int idx, char *par)
{
  char *nick, *chn = NULL;
  struct userrec *u1;
  struct chanuserrec *chanrec;
...
static void
cmd_cycle (struct userrec *u, int idx, char *par)
{
  char *chname;
  char buf2[1024];
...
static void
cmd_down (struct userrec *u, int idx, char *par)
{
  char *chname;
  char buf2[1024];
...
static void
cmd_pls_chan (struct userrec *u, int idx, char *par)
{
  char *chname;
  char buf2[1024];
  struct chanset_t *chan;
...
static void
cmd_mns_chan (struct userrec *u, int idx, char *par)
{
  char *chname;
  char buf2[1024];
  struct chanset_t *chan;
  ...
  if (!chan)
	...
	dprintf (idx,
		 "That channel exists with a short name of %s, use that.\n",
		 chan->dname);
      else
	dprintf (idx, "That channel doesn't exist!\n");
      return;
...
static void
cmd_chaninfo (struct userrec *u, int idx, char *par)
{
  ...
  if (!(chan = findchan_by_dname (chname)))
      ...
      if (u->flags & USER_OWNER)
	{
      ...
      dprintf (idx,
	       "     %cprotectops     %cprotectfriends %crevenge        %crevengebot\n",
	...
	if (ul->defined && ul->type == UDEF_INT)
	  {
	    ...
	    egg_snprintf (work + work_len, sizeof (work) - work_len,
			  "%s: %d   ", ul->name, getudef (ul->values,
							  chan->dname));
	    ii++;
	    if (ii > 4)
	      {
...
static void
cmd_chanset (struct userrec *u, int idx, char *par)
{
  ...
  if (!par[0])
      ...
      else
	{
	  ...
	  if (!chan
	      && !(chan =
		   findchan_by_dname (chname = dcc[idx].u.chat->con_chan)))
	    {
	      dprintf (idx, "Invalid console channel.\n");
	      return;
	    }
	  ...
	  while (list[0][0])
		  ...
		  || (!strcmp (list[0], "dont-idle-kick")))
		{
		  if (tcl_channel_modify (0, chan, 1, list) == TCL_OK)
		    ...
		    dprintf (idx,
			     "Error trying to set %s for %s, invalid mode.\n",
	      ...
	      if (strncmp (list[0], "need-", 5) || (u->flags & USER_OWNER))
		{
		  ...
		  if (tcl_channel_modify (0, chan, 2, list) == TCL_OK)
		    ...
		    dprintf (idx,
			     "Error trying to set %s for %s, invalid option\n",
			     list[0], all ? "all channels" : chname);
		  nfree (parcpy);
		}
	      break;
...
#ifdef S_IRCNET
{"+exempt", "o|o", (Function) cmd_pls_exempt, NULL, NULL}, {"+invite", "o|o",
							    (Function)
							    cmd_pls_invite,
							    NULL, NULL},
#endif
{"+chan", "n", (Function) cmd_pls_chan, NULL, NULL}, {"+chrec", "m|m",
						      (Function)
						      cmd_pls_chrec, NULL,
						      NULL}, {"-ban", "o|o",
							      (Function)
							      cmd_mns_ban,
							      NULL, NULL},
  {"-chan", "n", (Function) cmd_mns_chan, NULL, NULL}, {"-chrec", "m|m",
							(Function)
							cmd_mns_chrec, NULL,
							NULL},
#ifdef S_IRCNET
{"-exempt", "o|o", (Function) cmd_mns_exempt, NULL, NULL}, {"-invite", "o|o",
							    (Function)
							    cmd_mns_invite,
							    NULL, NULL},
#endif
...
#ifdef S_IRCNET
{"exempts", "o|o", (Function) cmd_exempts, NULL, NULL}, {"invites", "o|o",
							 (Function)
							 cmd_invites, NULL,
							 NULL},
#endif
{"chaninfo", "m|m", (Function) cmd_chaninfo, NULL, NULL}, {"chanset", "m|m",
							   (Function)
							   cmd_chanset, NULL,
							   NULL}, {"chinfo",
								   "m|m",
								   ...
								   NULL,
								   NULL},
  {"cycle", "n|n", (Function) cmd_cycle, NULL, NULL}, {"down", "n|n",
						       (Function) cmd_down,
						       NULL, NULL}, {"info",
								     "",
								     (Function)
								     ...
								     NULL,
								     NULL},
  {"slowjoin", "n", (Function) cmd_slowjoin, NULL, NULL}, {"stick", "o|o",
							   (Function)
							   cmd_stick, NULL,
							   NULL}, {"unstick",
								   "o|o",
