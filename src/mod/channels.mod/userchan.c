...
#endif
struct chanuserrec *
get_chanrec (struct userrec *u, char *chname)
{
...
add_chanrec (struct userrec *u, char *chname)
{
...
static void
add_chanrec_by_handle (struct userrec *bu, char *hand, char *chname)
{
  struct userrec *u;
...
static void
get_handle_chaninfo (char *handle, char *chname, char *s)
{
  struct userrec *u;
  struct chanuserrec *ch;
...
static void
set_handle_chaninfo (struct userrec *bu, char *handle, char *chname,
		     char *info)
{
  struct userrec *u;
  struct chanuserrec *ch;
...
static void
del_chanrec (struct userrec *u, char *chname)
{
...
static void
set_handle_laston (char *chan, struct userrec *u, time_t n)
{
  struct chanuserrec *ch;
  if (!u)
    return;
...
static int
u_sticky_mask (maskrec * u, char *uhost)
{
  for (; u; u = u->next)
...
static int
u_setsticky_mask (struct chanset_t *chan, maskrec * u, char *uhost,
...
static int
u_equals_mask (maskrec * u, char *mask)
{
  for (; u; u = u->next)
...
static int
u_match_mask (maskrec * rec, char *mask)
{
...
static int
u_delban (struct chanset_t *c, char *who, int doit)
{
  int j, i = 0;
  maskrec *t;
...
static int
u_delexempt (struct chanset_t *c, char *who, int doit)
{
  int j, i = 0;
  maskrec *t;
...
static int
u_delinvite (struct chanset_t *c, char *who, int doit)
{
  int j, i = 0;
  maskrec *t;
...
static int
u_addban (struct chanset_t *chan, char *ban, char *from, char *note,
...
static int
u_addinvite (struct chanset_t *chan, char *invite, char *from, char *note,
	     time_t expire_time, int flags)
{
  char host[1024], s[1024];
      ...
      if (mask)
	{
	  if (!chan)
	    shareout (NULL, "+inv %s %lu %s%s %s %s\n", mask,
		      expire_time - now, (flags & MASKREC_STICKY) ? "s" : "",
		      (flags & MASKREC_PERM) ? "p" : "-", from, note);
	  else
...
static int
u_addexempt (struct chanset_t *chan, char *exempt, char *from, char *note,
	     time_t expire_time, int flags)
{
  char host[1024], s[1024];
...
static void
display_ban (int idx, int number, maskrec * ban, struct chanset_t *chan,
	     int show_inact)
{
  char dates[81], s[41];
  if (ban->added)
...
static void
display_exempt (int idx, int number, maskrec * exempt, struct chanset_t *chan,
		int show_inact)
{
  char dates[81], s[41];
  if (exempt->added)
...
static void
display_invite (int idx, int number, maskrec * invite, struct chanset_t *chan,
		int show_inact)
{
  char dates[81], s[41];
  if (invite->added)
...
static void
tell_bans (int idx, int show_inact, char *match)
{
  int k = 1;
  char *chname;
  ...
  for (u = global_bans; u; u = u->next)
    {
      if (match[0])
	{
	  if ((wild_match (match, u->mask)) || (wild_match (match, u->desc))
...
static void
tell_exempts (int idx, int show_inact, char *match)
{
  int k = 1;
  char *chname;
  struct chanset_t *chan = NULL;
  ...
  if (!chan && !(chan = findchan_by_dname (dcc[idx].u.chat->con_chan))
    ...
    chan = NULL;
  if (chan && show_inact)
    dprintf (idx, "%s:   (! = %s %s)\n", EXEMPTS_GLOBAL, MODES_NOTACTIVE,
	     chan->dname);
  else
    dprintf (idx, "%s:\n", EXEMPTS_GLOBAL);
...
static void
tell_invites (int idx, int show_inact, char *match)
{
  int k = 1;
  char *chname;
  struct chanset_t *chan = NULL;
  ...
  if (!chan && !(chan = findchan_by_dname (dcc[idx].u.chat->con_chan))
    ...
    chan = NULL;
  if (chan && show_inact)
    dprintf (idx, "%s:   (! = %s %s)\n", INVITES_GLOBAL, MODES_NOTACTIVE,
	     chan->dname);
  else
    dprintf (idx, "%s:\n", INVITES_GLOBAL);
	...
	for (ir = chan->invites; ir; ir = ir->next)
	  {
	    ...
	    if (!mask
		|| lfprintf (f, "@ %s:%s%lu%s:+%lu:%lu:%s:%s\n", ir->mask,
			     (ir->flags & MASKREC_PERM) ? "+" : "",
			     ir->expire,
			     (ir->flags & MASKREC_STICKY) ? "*" : "",
			     ir->added, ir->lastactive,
			     ir->user ? ir->user : botnetnick,
			     ir->desc ? ir->desc : "requested") == EOF)
	      {
		if (mask)
		  nfree (mask);
  ...
  for (chan = chanset; chan; chan = chan->next)
      ...
      if ((idx < 0) || (1))
	{
	  ...
	  if (idx >= 0)
	    ...
	    fr.chan = BOT_SHARE;
	  putlog (LOG_DEBUG, "*", "writing channel %s to userfile..",
		  chan->dname);
	  memset (udefs, '\0', 2048);
	  ...
	  for (ul = udef; ul; ul = ul->next)
	      ...
	      if (ul->defined && ul->name)
		{
		  if (ul->type == UDEF_FLAG)
		    ...
		    sprintf (buf, "%s%s %d ", "udef-int-", ul->name,
			     getudef (ul->values, chan->dname));
		  else
	  ...
	  if (lfprintf
	      (f,
	       ...
	       chan->revenge_mode, chan->flood_pub_thr, chan->flood_pub_time,
	       chan->flood_ctcp_thr, chan->flood_ctcp_time,
	       chan->flood_join_thr, chan->flood_join_time,
	       chan->flood_kick_thr, chan->flood_kick_time,
	       chan->flood_deop_thr, chan->flood_deop_time,
	       chan->flood_nick_thr, chan->flood_nick_time, chan->ban_time,
#ifdef S_IRCNET
...
static int
expired_mask (struct chanset_t *chan, char *who)
{
  memberlist *m, *m2;
...
check_expired_bans (void)
{
  ...
  for (u = global_bans; u; u = u2)
	...
	{
	  putlog (LOG_MISC, "*", "%s %s (%s)", BANS_NOLONGER, u->mask,
		  MISC_EXPIRED);
	  for (chan = chanset; chan; chan = chan->next)
  ...
  for (u = global_exempts; u; u = u2)
    {
      ...
      if (!(u->flags & MASKREC_PERM) && (now >= u->expire))
	{
	  putlog (LOG_MISC, "*", "%s %s (%s)", EXEMPTS_NOLONGER, u->mask,
		  MISC_EXPIRED);
	  for (chan = chanset; chan; chan = chan->next)
	    {
	      match = 0;
  ...
  for (u = global_invites; u; u = u2)
    {
      ...
      if (!(u->flags & MASKREC_PERM) && (now >= u->expire))
	{
	  putlog (LOG_MISC, "*", "%s %s (%s)", INVITES_NOLONGER, u->mask,
		  MISC_EXPIRED);
	  for (chan = chanset; chan; chan = chan->next)
