...
static int dcc_limit = 4;
static int dcc_block = 0;
static int quiet_reject = 1;
...
#define NOMATCH 0
static int
wild_match_file (register char *m, register char *n)
{
...
static void
wipe_tmp_filename (char *fn, int idx)
{
  int i, ok = 1;
  if (!copy_to_tmp)
    return;
...
static void
check_tcl_sentrcvd (struct userrec *u, char *nick, char *path,
		    p_tcl_bind_list h)
{
...
check_tcl_toutlost (struct userrec *u, char *nick, char *path,
...
static void
queue_file (char *dir, char *file, char *from, char *to)
{
  fileq_t *q = fileq;
  ...
  for (i = 0; i < dcc_total; i++)
	      ...
	      || !egg_strcasecmp (dcc[i].u.xfer->from, dcc[idx].nick)))
	{
	  ...
	  if (dcc[i].type == &DCC_GET_PENDING)
	    ...
	    dprintf (idx, TRANSFER_DONE, dcc[i].nick, spaces, nfn,
		     (100.0 *
		      ((float) dcc[i].status /
...
fileq_cancel (int idx, char *par)
{
  ...
  if (!matches)
    ...
    dprintf (idx, TRANSFER_CANCELLED_FILE, matches,
	     (matches != 1) ? "s" : "");
  for (i = 0; i < atot; i++)
...
static int tcl_getfileq STDVAR
{
  ...
  for (q = fileq; q; q = q->next)
      ...
      if (!egg_strcasecmp (q->nick, argv[1]))
	{
	  s =
	    nrealloc (s,
		      strlen (q->to) + strlen (q->dir) + strlen (q->file) +
		      4);
	  if (q->dir[0] == '*')
	    sprintf (s, "%s %s/%s", q->to, &q->dir[1], q->file);
	  else
	    sprintf (s, "%s /%s%s%s", q->to, q->dir, q->dir[0] ? "/" : "",
		     q->file);
...
static tcl_cmds mytcls[] =
  { {"dccsend", tcl_dccsend}, {"getfileq", tcl_getfileq}, {"getfilesendtime",
							   tcl_getfilesendtime},
  {NULL, NULL} };
#define	PMAX_SIZE	4096
static unsigned long
pump_file_to_sock (FILE * file, long sock,
...
eof_dcc_fork_send (int idx)
{
  ...
  if (!strcmp (dcc[idx].nick, "*users"))
	...
	dprintf (DP_HELP, "NOTICE %s :%s (%s)\n", dcc[idx].nick,
      ...
      s2 =
	nmalloc (strlen (tempdir) + strlen (dcc[idx].u.xfer->filename) + 1);
...
eof_dcc_send (int idx)
{
  ...
  if (dcc[idx].u.xfer->length == dcc[idx].status)
	...
	nmalloc (strlen (tempdir) + strlen (dcc[idx].u.xfer->filename) + 1);
      nfn =
	nmalloc (strlen (dcc[idx].u.xfer->dir) +
...
static inline u_8bit_t
byte_order_test (void)
{
...
inline static void
handle_resend_packet (int idx, transfer_reget * reget_data)
{
  if (byte_order_test () != reget_data->byte_order)
    {
      reget_data->packet_id =
	((reget_data->packet_id & 0x00ff) << 8) | ((reget_data->
						    packet_id & 0xff00) >> 8);
      reget_data->byte_offset =
	((reget_data->byte_offset & 0xff000000) >> 24) | ((reget_data->
							   byte_offset &
							   0x00ff0000) >> 8) |
	((reget_data->byte_offset & 0x0000ff00) << 8) | ((reget_data->
							  byte_offset &
							  0x000000ff) << 24);
    }
...
static void
dcc_get (int idx, char *buf, int len)
{
  char xnick[NICKLEN];
  ...
  if (w < 4 || (w < 8 && dcc[idx].u.xfer->type == XFER_RESEND_PEND))
      ...
      if (w == 8)
	{
	  ...
	  my_memcpy (&reget_data, dcc[idx].u.xfer->buf,
		     dcc[idx].u.xfer->sofar);
      ...
      else
	{
      ...
      cmp =
	((unsigned int) (bbuf[0]) << 24) + ((unsigned int) (bbuf[1]) << 16) +
  ...
  if (cmp > dcc[idx].u.xfer->length && cmp > dcc[idx].status)
    {
      putlog (LOG_FILES, "*", TRANSFER_BEHIND_FILEEND,
	...
	{
	  putlog (LOG_BOTS, "*",
		  "!!! Trying to skip ahead on binary transfer");
	}
      else
	{
  ...
  if (dcc[idx].status == dcc[idx].u.xfer->length)
      ...
      else
	{
	  module_entry *fs = module_find ("filesys", 0, 0);
	  struct userrec *u =
	    get_user_by_handle (userlist, dcc[idx].u.xfer->from);
...
static void
dcc_send (int idx, char *buf, int len)
{
  char s[512], *b;
  unsigned long sent;
  ...
  if (dcc[idx].status > dcc[idx].u.xfer->length
      && dcc[idx].u.xfer->length > 0)
    {
      ...
      putlog (LOG_FILES, "*", TRANSFER_FILE_TOO_LONG,
	      dcc[idx].u.xfer->origname, dcc[idx].nick, dcc[idx].host);
      fclose (dcc[idx].u.xfer->f);
...
transfer_get_timeout (int i)
{
  ...
  if (strcmp (dcc[i].nick, "*users") == 0)
      ...
      if (y != 0)
	{
      ...
      dprintf (y, "bye\n");
      egg_snprintf (xx, sizeof xx,
		    "Disconnected %s (timed-out binary transfer)",
		    dcc[y].nick);
      botnet_send_unlinked (y, dcc[y].nick, xx);
      chatout ("*** %s\n", xx);
      if (y < i)
  ...
  else
    {
      ...
      check_tcl_toutlost (u, dcc[i].nick, dcc[i].u.xfer->dir,
			  dcc[i].u.xfer->acked, dcc[i].u.xfer->length,
			  H_tout);
...
display_dcc_get (int idx, char *buf)
{
  if (dcc[idx].status == dcc[idx].u.xfer->length)
...
static void
display_dcc_get_p (int idx, char *buf)
{
  sprintf (buf, TRANSFER_SEND_WAITED, now - dcc[idx].timeval,
	   dcc[idx].u.xfer->origname);
} static void
display_dcc_send (int idx, char *buf)
{
  Context;
  sprintf (buf, TRANSFER_SEND, dcc[idx].status, dcc[idx].u.xfer->length,
	   dcc[idx].u.xfer->origname);
} static void
display_dcc_fork_send (int idx, char *buf)
{
  sprintf (buf, TRANSFER_CONN_SEND);
} static int
expmem_dcc_xfer (void *x)
{
...
static struct dcc_table DCC_SEND =
  { "SEND", DCT_FILETRAN | DCT_FILESEND | DCT_VALIDIDX, eof_dcc_send,
dcc_send, &wait_dcc_xfer, tout_dcc_send, display_dcc_send, expmem_dcc_xfer, kill_dcc_xfer, out_dcc_xfer };
static void dcc_fork_send (int idx, char *x, int y);
static struct dcc_table DCC_FORK_SEND =
  { "FORK_SEND", DCT_FILETRAN | DCT_FORKTYPE | DCT_FILESEND | DCT_VALIDIDX,
eof_dcc_fork_send, dcc_fork_send, &wait_dcc_xfer, eof_dcc_fork_send, display_dcc_fork_send, expmem_dcc_xfer,
kill_dcc_xfer, out_dcc_xfer };
static void
dcc_fork_send (int idx, char *x, int y)
{
  char s1[121];
...
static struct dcc_table DCC_GET =
  { "GET", DCT_FILETRAN | DCT_VALIDIDX, eof_dcc_get, dcc_get, &wait_dcc_xfer,
transfer_get_timeout, display_dcc_get, expmem_dcc_xfer, kill_dcc_xfer, out_dcc_xfer, outdone_dcc_xfer };
static struct dcc_table DCC_GET_PENDING =
  { "GET_PENDING", DCT_FILETRAN | DCT_VALIDIDX, eof_dcc_get, dcc_get_pending,
&wait_dcc_xfer, transfer_get_timeout, display_dcc_get_p, expmem_dcc_xfer, kill_dcc_xfer, out_dcc_xfer };
static void
dcc_get_pending (int idx, char *buf, int len)
{
  unsigned long ip;
  unsigned short port;
  int i;
...
static int
raw_dcc_resend_send (char *filename, char *nick, char *from, char *dir,
		     int resend)
{
  int zz, port, i;
  char *nfn, *buf = NULL;
...
static int
raw_dcc_resend (char *filename, char *nick, char *from, char *dir)
{
  return raw_dcc_resend_send (filename, nick, from, dir, 1);
}
static int
raw_dcc_send (char *filename, char *nick, char *from, char *dir)
{
  return raw_dcc_resend_send (filename, nick, from, dir, 0);
}
static tcl_ints myints[] =
  { {"max-dloads", &dcc_limit}, {"dcc-block", &dcc_block}, {"copy-to-tmp",
							    &copy_to_tmp},
  {"xfer-timeout", &wait_dcc_xfer}, {NULL, NULL} };
static int
fstat_unpack (struct userrec *u, struct user_entry *e)
{
  char *par, *arg;
...
static int
fstat_write_userfile (FILE * f, struct userrec *u, struct user_entry *e)
{
  register struct filesys_stats *fs;
  fs = e->u.extra;
  if (lfprintf
      (f, "--FSTAT %09u %09u %09u %09u\n", fs->uploads, fs->upload_ks,
       fs->dnloads, fs->dnload_ks) == EOF)
    return 0;
  return 1;
}
static int
fstat_set (struct userrec *u, struct user_entry *e, void *buf)
{
...
static int
fstat_tcl_get (Tcl_Interp * irp, struct userrec *u, struct user_entry *e,
	       int argc, char **argv)
{
  register struct filesys_stats *fs;
  char d[50];
...
static int
fstat_kill (struct user_entry *e)
{
  if (e->u.extra)
    nfree (e->u.extra);
  nfree (e);
...
static int
fstat_expmem (struct user_entry *e)
{
  return sizeof (struct filesys_stats);
} static void
fstat_display (int idx, struct user_entry *e)
{
  struct filesys_stats *fs;
  fs = e->u.extra;
...
static struct user_entry_type USERENTRY_FSTAT =
  { NULL, fstat_gotshare, fstat_dupuser, fstat_unpack, fstat_pack,
fstat_write_userfile, fstat_kill, NULL, fstat_set, fstat_tcl_get, fstat_tcl_set, fstat_expmem, fstat_display, "FSTAT" };
static int
fstat_gotshare (struct userrec *u, struct user_entry *e, char *par, int idx)
{
  char *p;
  struct filesys_stats *fs;
  noshare = 1;
...
static int
fstat_dupuser (struct userrec *u, struct userrec *o, struct user_entry *e)
{
  struct filesys_stats *fs;
  if (e->u.extra)
    {
...
static void
stats_add_dnload (struct userrec *u, unsigned long bytes)
{
...
stats_add_upload (struct userrec *u, unsigned long bytes)
{
  struct user_entry *ue;
  register struct filesys_stats *fs;
  if (u)
...
fstat_tcl_set (Tcl_Interp * irp, struct userrec *u, struct user_entry *e,
	       int argc, char **argv)
{
  register struct filesys_stats *fs;
  int f = 0, k = 0;
...
static int
ctcp_DCC_RESUME (char *nick, char *from, char *handle, char *object,
...
static cmd_t transfer_ctcps[] =
  { {"DCC", "", ctcp_DCC_RESUME, "transfer:DCC"}, {NULL, NULL, NULL, NULL} };
static int
server_transfer_setup (char *mod)
{
...
static cmd_t transfer_load[] =
  { {"server", "", server_transfer_setup, NULL}, {NULL, "", NULL, NULL} };
static char *
transfer_close ()
{
  int i;
...
static void
transfer_report (int idx, int details)
{
  if (details)
    {
      dprintf (idx, TRANSFER_STAT_BLOCK, dcc_block,
...
(Function) transfer_expmem, (Function) transfer_report, (Function) & DCC_FORK_SEND, (Function) at_limit,
(Function) & copy_to_tmp, (Function) fileq_cancel, (Function) queue_file, (Function) raw_dcc_send,
(Function) show_queued_files, (Function) wild_match_file, (Function) wipe_tmp_filename,
(Function) & DCC_GET, (Function) & H_rcvd, (Function) & H_sent, (Function) & USERENTRY_FSTAT,
(Function) & quiet_reject, (Function) raw_dcc_resend, (Function) & H_lost, (Function) & H_tout, };
char *
transfer_start (Function * global_funcs)
{
  global = global_funcs;
  fileq = NULL;
