...
static int
console_pack (struct userrec *u, struct user_entry *e)
{
  char work[1024];
  ...
  l =
    simple_sprintf (work, "%s %s %s %d %d %d %d", ci->channel,
		    masktype (ci->conflags), stripmasktype (ci->stripflags),
		    ci->echoflags, ci->page, ci->conchan, ci->color);
...
static int
console_tcl_get (Tcl_Interp * irp, struct userrec *u, struct user_entry *e,
		 int argc, char **argv)
{
  char work[1024];
  struct console_info *i = e->u.extra;
  simple_sprintf (work, "%s %s %s %d %d %d %d", i->channel,
		  masktype (i->conflags), stripmasktype (i->stripflags),
		  i->echoflags, i->page, i->conchan, i->color);
  Tcl_AppendResult (irp, work, NULL);
  return TCL_OK;
}
static int
console_tcl_set (Tcl_Interp * irp, struct userrec *u, struct user_entry *e,
		 int argc, char **argv)
{
  struct console_info *i = e->u.extra;
  int l;
...
static int
console_expmem (struct user_entry *e)
{
  struct console_info *i = e->u.extra;
  return sizeof (struct console_info) + strlen (i->channel) + 1;
} static void
console_display (int idx, struct user_entry *e)
{
  struct console_info *i = e->u.extra;
...
static int
console_dupuser (struct userrec *new, struct userrec *old,
		 struct user_entry *e)
{
...
static struct user_entry_type USERENTRY_CONSOLE =
  { NULL, NULL, console_dupuser, console_unpack, console_pack,
console_write_userfile, console_kill, NULL, console_set, console_tcl_get, console_tcl_set, console_expmem,
...
static tcl_ints myints[] =
  { {"console-autosave", &console_autosave, 0}, {"force-channel",
						 &force_channel, 0},
  {"info-party", &info_party, 0}, {NULL, NULL, 0} };
static cmd_t mychon[] =
  { {"*", "", console_chon, "console:chon"}, {NULL, NULL, NULL, NULL} };
static dcc_cmd_t mydcc[] =
  { {"store", "", console_store, NULL, NULL}, {NULL, NULL, NULL, NULL,
					       NULL} };
static char *
console_close ()
{
...
char *
console_start (Function * global_funcs)
{
  global = global_funcs;
