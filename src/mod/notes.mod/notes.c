...
static int allow_fwd = 1;
static int notify_users = 1;
...
#include "cmdsnote.c"
static void
fwd_display (int idx, struct user_entry *e)
{
...
static void
notes_change (char *oldnick, char *newnick)
{
  FILE *f, *g;
  char s[513], *to, *s1;
  int tot = 0;
...
static void
notes_read (char *hand, char *nick, char *srd, int idx)
{
  FILE *f;
	  ...
	  if ((s[0]) && (s[0] != '#') && (s[0] != ';'))
	      ...
	      if (!egg_strcasecmp (to, hand))
		{
		  ...
		  if (lapse > note_life - 7)
			...
			sprintf (&dt[strlen (dt)], NOTES_EXPIRE_XDAYS,
				 note_life - lapse,
				 (note_life - lapse) == 1 ? "" : "S");
		    }
		  if (srd[0] == '+')
...
static void
notes_del (char *hand, char *nick, char *sdl, int idx)
{
  FILE *f, *g;
  char s[513], *to, *s1;
  int in = 1;
...
static int
msg_notes (char *nick, char *host, struct userrec *u, char *par)
{
  char *pwd, *fcn;
  if (!u)
    return 0;
  ...
  if (!par[0])
    {
      ...
      dprintf (DP_HELP, "NOTICE %s :       %s\n", nick, NOTES_MAYBE);
      dprintf (DP_HELP,
	       "NOTICE %s :       ex: NOTES mypass ERASE 2-4;8;16-\n", nick);
      return 1;
    }
  if (!u_pass_match (u, "-"))
    {
  ...
  if (!egg_strcasecmp (fcn, "INDEX"))
	...
	{
	  dprintf (DP_HELP,
		   "NOTICE %s :%s: NOTES <pass> TO <hand> <message>\n", nick,
		   NOTES_USAGE);
	  return 0;
	}
    ...
    dprintf (DP_HELP, "NOTICE %s :%s INDEX, READ, ERASE, TO\n", nick,
	     NOTES_DCC_USAGE_READ);
  putlog (LOG_CMDS, "*", "(%s!%s) !%s! NOTES %s %s", nick, host, u->handle,
	  fcn, par[0] ? "..." : "");
  return 1;
}
static void
	  ...
	  for (m = chan->channel.member; m && m->nick[0]; m = m->next)
	      ...
	      if (u)
		{
		  ...
		  if (k)
		    {
		      dprintf (DP_HELP,
			       "NOTICE %s :You have %d note%s waiting on %s.\n",
			       m->nick, k, k == 1 ? "" : "s", botname);
		      dprintf (DP_HELP,
			       "NOTICE %s :%s /MSG %s NOTES <pass> INDEX\n",
...
static void
away_notes (char *bot, int sock, char *msg)
{
...
static void
join_notes (char *nick, char *uhost, char *handle, char *par)
{
  int i = -1, j;
...
static struct xtra_key *
getnotesentry (struct userrec *u)
{
...
int
get_note_ignores (struct userrec *u, char ***ignores)
{
  struct xtra_key *xk;
  char *buf, *p;
  int ignoresn;
...
int
add_note_ignore (struct userrec *u, char *mask)
{
  struct xtra_key *xk;
  char **ignores;
  int ignoresn, i;
  ...
  if (!xk)
    ...
    {
      xk->data =
	user_realloc (xk->data, strlen (xk->data) + strlen (mask) + 2);
      strcat (xk->data, " ");
      strcat (xk->data, mask);
    }
  return 1;
...
int
del_note_ignore (struct userrec *u, char *mask)
{
...
int
match_note_ignore (struct userrec *u, char *from)
{
  char **ignores;
  int ignoresn, i;
  ignoresn = get_note_ignores (u, &ignores);
...
static cmd_t notes_join[] =
  { {"*", "", (Function) join_notes, "notes"}, {NULL, NULL, NULL, NULL} };
static cmd_t notes_nkch[] =
  { {"*", "", (Function) notes_change, "notes"}, {NULL, NULL, NULL, NULL} };
static cmd_t notes_away[] =
  { {"*", "", (Function) away_notes, "notes"}, {NULL, NULL, NULL, NULL} };
static cmd_t notes_chon[] =
  { {"*", "", (Function) chon_notes, "notes"}, {NULL, NULL, NULL, NULL} };
static cmd_t notes_msgs[] =
  { {"notes", "", (Function) msg_notes, NULL}, {NULL, NULL, NULL, NULL} };
static tcl_ints notes_ints[] =
  { {"note-life", &note_life}, {"max-notes", &maxnotes}, {"allow-fwd",
							  &allow_fwd},
  {"notify-users", &notify_users}, {"notify-onjoin", &notify_onjoin}, {NULL,
								       NULL}
};
static tcl_strings notes_strings[] =
  { {"notefile", notefile, 120, 0}, {NULL, NULL, 0, 0} };
static tcl_cmds notes_tcls[] =
  { {"notes", tcl_notes}, {"erasenotes", tcl_erasenotes}, {"listnotes",
							   tcl_listnotes},
  {"storenote", tcl_storenote}, {NULL, NULL} };
static int
notes_irc_setup (char *mod)
{
...
static cmd_t notes_load[] =
  { {"server", "", notes_server_setup, "notes:server"}, {"irc", "",
							 notes_irc_setup,
							 "notes:irc"}, {NULL,
									NULL,
									NULL,
...
static void
notes_report (int idx, int details)
{
  if (details)
    {
      if (notefile[0])
...
static Function notes_table[] =
  { (Function) notes_start, (Function) notes_close, (Function) notes_expmem,
(Function) notes_report, (Function) cmd_note, };
char *
notes_start (Function * global_funcs)
{
  global = global_funcs;
  notefile[0] = 0;
