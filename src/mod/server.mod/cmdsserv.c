#ifdef LEAF
static void
cmd_servers (struct userrec *u, int idx, char *par)
{
...
static void
cmd_dump (struct userrec *u, int idx, char *par)
{
...
static void
cmd_jump (struct userrec *u, int idx, char *par)
{
  char *other;
  int port;
  if (par[0])
...
static void
cmd_clearqueue (struct userrec *u, int idx, char *par)
{
  int msgs;
  if (!par[0])
    {
...
static dcc_cmd_t C_dcc_serv[] =
  { {"clearqueue", "m", (Function) cmd_clearqueue, NULL, NULL}, {"dump", "a",
								 (Function)
								 cmd_dump,
								 NULL, NULL},
  {"jump", "m", (Function) cmd_jump, NULL, NULL}, {"servers", "m",
