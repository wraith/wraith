...
static unsigned int share_compressed = 1;
static unsigned int compress_level = 9;
...
static int
is_compressedfile (char *filename)
{
...
static int
uncompress_to_file (char *f_src, char *f_target)
{
  char buf[BUFLEN];
  int len;
  FILE *fin, *fout;
  ...
  if (!fout)
    {
      putlog (LOG_MISC, "*",
	      "Failed to uncompress file `%s': open failed: %s.", f_src,
	      strerror (errno));
      return COMPF_ERROR;
    }
  while (1)
	...
	{
	  putlog (LOG_MISC, "*",
		  "Failed to uncompress file `%s': gzread failed.", f_src);
	  return COMPF_ERROR;
	}
      if (!len)
	break;
  ...
  if (fclose (fout))
    {
      putlog (LOG_MISC, "*",
	      "Failed to uncompress file `%s': fclose failed: %s.", f_src,
	      strerror (errno));
      return COMPF_ERROR;
    }
  if (gzclose (fin) != Z_OK)
    {
      putlog (LOG_MISC, "*",
	      "Failed to uncompress file `%s': gzclose failed.", f_src);
      return COMPF_ERROR;
    }
  uncompressed_files++;
...
#ifdef HAVE_MMAP
static int
compress_to_file_mmap (FILE * fout, FILE * fin)
{
...
#endif
static int
compress_to_file (char *f_src, char *f_target, int mode_num)
{
  ...
  while (1)
	...
	{
	  putlog (LOG_MISC, "*",
		  "Failed to compress file `%s': fread failed: %s", f_src,
		  strerror (errno));
	  return COMPF_ERROR;
	}
      if (!len)
	...
	{
	  putlog (LOG_MISC, "*",
		  "Failed to compress file `%s': gzwrite failed.", f_src);
	  return COMPF_ERROR;
	}
    }
  fclose (fin);
...
static tcl_ints my_tcl_ints[] =
  { {"share-compressed", &share_compressed}, {"compress-level",
					      &compress_level}, {NULL,
								 NULL} };
static int
compress_expmem (void)
{
  return 0;
}
static int
compress_report (int idx, int details)
{
  if (details)
...
static Function compress_table[] =
  { (Function) compress_start, (Function) compress_close,
(Function) compress_expmem, (Function) compress_report, (Function) compress_to_file,
(Function) compress_file, (Function) uncompress_to_file, (Function) uncompress_file,
(Function) is_compressedfile, };
char *
compress_start (Function * global_funcs)
{
  global = global_funcs;
  compressed_files = 0;
