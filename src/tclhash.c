...
p_tcl_bind_list H_chat, H_act, H_bcst, H_chon, H_chof, H_load, H_unld, H_link,
  H_disc, H_dcc, H_chjn, H_chpt, H_bot, H_time, H_nkch, H_away, H_note,
  H_filt, H_event;
...
static inline void *
n_malloc_null (int size, const char *file, int line)
{
#ifdef DEBUG_MEM
...
static inline void
tcl_cmd_delete (tcl_cmd_t * tc)
{
  nfree (tc->func_name);
  nfree (tc);
} static inline void

tcl_bind_mask_delete (tcl_bind_mask_t * tm)
{
...
static inline void
tcl_bind_list_delete (tcl_bind_list_t * tl)
{
...
static inline int
tcl_bind_mask_expmem (tcl_bind_mask_t * tm)
{
  int tot = 0;
  tcl_cmd_t *tc;
...
static inline int
tcl_bind_list_expmem (tcl_bind_list_t * tl)
{
  int tot = 0;
  tcl_bind_mask_t *tm;
...
static cd_tcl_cmd cd_cmd_table[] =
  { {"bind", tcl_bind, (void *) 0}, {"unbind", tcl_bind, (void *) 1}, {0} };
void
init_bind (void)
{
...
tcl_bind_list_t *
add_bind_table (const char *nme, int flg, Function func)
{
...
void
del_bind_table (tcl_bind_list_t * tl_which)
{
  tcl_bind_list_t *tl;
  for (tl = bind_table_list; tl; tl = tl->next)
    {
...
tcl_bind_list_t *
find_bind_table (const char *nme)
{
  tcl_bind_list_t *tl;
  int v, i = 0;
  for (tl = bind_table_list; tl; tl = tl->next)
...
static void
dump_bind_tables (Tcl_Interp * irp)
{
  tcl_bind_list_t *tl;
  u_8bit_t i;
...
static int
unbind_bind_entry (tcl_bind_list_t * tl, const char *flags, const char *cmd,
		   const char *proc)
{
  tcl_bind_mask_t *tm;
  for (tm = tl->first; tm; tm = tm->next)
...
static int
bind_bind_entry (tcl_bind_list_t * tl, const char *flags, const char *cmd,
		 const char *proc)
{
  tcl_cmd_t *tc;
...
static int
tcl_getbinds (tcl_bind_list_t * tl_kind, const char *name)
{
  tcl_bind_mask_t *tm;
...
int
check_validity (char *nme, Function func)
{
  char *p;
  tcl_bind_list_t *tl;
  if (*nme != '*')
...
static int builtin_5int STDVAR
{
  Function F = (Function) cd;
    ...
    F (atoi (argv[1]), atoi (argv[2]), atoi (argv[3]), atoi (argv[4]),
       atoi (argv[5]));
    return TCL_OK;
}
...
static int
trigger_bind (const char *proc, const char *param)
{
  int x;
  {
    ...
    char *buf;
    buf =
      nmalloc (strlen (msg) + (proc ? strlen (proc) : 6) +
...
int
check_tcl_bind (tcl_bind_list_t * tl, const char *match,
		struct flag_record *atr, const char *param, int match_type)
{
  ...
  for (tm = tl->first; tm && !finish; tm_last = tm, tm = tm->next)
	...
	case MATCH_MASK:
	  ok =
	    wild_match_per ((unsigned char *) tm->mask,
			    (unsigned char *) match);
	  break;
	default:
	  ok = 0;
      ...
      if (match_type & BIND_STACKABLE)
	{
	  for (tc = tm->first; tc; tc = tc->next)
	      ...
	      if (atrok)
		{
		  ...
		  tc->hits++;
		  tm_p = tm_last;
		  Tcl_SetVar (interp, "lastbind", (char *) tm->mask,
			      TCL_GLOBAL_ONLY);
...
int
check_tcl_dcc (char *cmd, int idx, char *args)
{
  ...
  if (has_cmd_pass (cmd))
	...
	*p = 0;
	...
	{
	  dprintf (idx,
		   "Invalid command password. Use %scommand password arguments\n",
		   dcc_prefix);
	  putlog (LOG_MISC, "*",
		  "%s attempted %s%s with missing or incorrect command password",
		  dcc[idx].nick, dcc_prefix, cmd);
	  return 0;
	}
    }
#endif
  ...
  x =
    check_tcl_bind (H_dcc, cmd, &fr, " $_dcc1 $_dcc2 $_dcc3",
...
void
check_tcl_bot (const char *nick, const char *code, const char *param)
{
...
check_tcl_chonof (char *hand, int sock, tcl_bind_list_t * tl)
{
  struct flag_record fr = { FR_GLOBAL | FR_CHAN, 0, 0, 0, 0, 0 };
  char s[11];
  struct userrec *u;
...
check_tcl_chatactbcst (const char *from, int chan, const char *text,
		       tcl_bind_list_t * tl)
{
  char s[11];
...
check_tcl_nkch (const char *ohand, const char *nhand)
{
...
check_tcl_link (const char *bot, const char *via)
{
...
check_tcl_filt (int idx, const char *text)
{
  ...
  x =
    check_tcl_bind (H_filt, text, &fr, " $_filt1 $_filt2",
		    MATCH_MASK | BIND_USE_ATTR | BIND_STACKABLE | BIND_WANTRET
		    | BIND_ALTER_ARGS);
...
int
check_tcl_note (const char *from, const char *to, const char *text)
{
  int x;
...
void
check_tcl_listen (const char *cmd, int idx)
{
  char s[11];
  int x;
...
void
check_tcl_chjn (const char *bot, const char *nick, int chan, const char type,
...
check_tcl_chpt (const char *bot, const char *hand, int sock, int chan)
{
  char u[11], v[11];
  egg_snprintf (u, sizeof u, "%d", sock);
...
check_tcl_away (const char *bot, int idx, const char *msg)
{
  char u[11];
		  ...
		  if (ok)
		    {
		      dprintf (idx, "  %-4s %-8s %-20s %4d %s\n", tl->name,
			       flg, tm->mask, tc->hits, tc->func_name);
		      fnd = 1;
		    }
		}
	    }
...
void
add_builtins (tcl_bind_list_t * tl, cmd_t * cc)
{
  int k, i;
  char p[1024], *l;
  cd_tcl_cmd table[2];
...
add_builtins_dcc (tcl_bind_list_t * tl, dcc_cmd_t * cc)
{
  int k, i;
  char p[1024], *l;
  cd_tcl_cmd table[2];
...
rem_builtins_dcc (tcl_bind_list_t * table, dcc_cmd_t * cc)
{
  int i;
  for (i = 0; cc[i].name; i++)
    {
...
void
rem_builtins (tcl_bind_list_t * table, cmd_t * cc)
{
  int k, i;
  char p[1024], *l;
  for (i = 0; cc[i].name; i++)
