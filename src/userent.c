...
list_type_kill (struct list_type *t)
{
  struct list_type *u;
  while (t)
    {
...
int
list_type_expmem (struct list_type *t)
{
  int tot = 0;
  for (; t; t = t->next)
...
int
def_unpack (struct userrec *u, struct user_entry *e)
{
  char *tmp;
  tmp = e->u.list->extra;
  e->u.list->extra = NULL;
...
int
def_pack (struct userrec *u, struct user_entry *e)
{
  char *tmp;
  tmp = e->u.string;
...
int
def_write_userfile (FILE * f, struct userrec *u, struct user_entry *e)
{
...
int
def_set (struct userrec *u, struct user_entry *e, void *buf)
{
...
int
def_tcl_get (Tcl_Interp * interp, struct userrec *u, struct user_entry *e,
	     int argc, char **argv)
{
...
int
def_tcl_set (Tcl_Interp * irp, struct userrec *u, struct user_entry *e,
	     int argc, char **argv)
{
...
int
def_expmem (struct user_entry *e)
{
  return strlen (e->u.string) + 1;
}

void
def_display (int idx, struct user_entry *e)
{
  dprintf (idx, "  %s: %s\n", e->type->name, e->u.string);
} int
def_dupuser (struct userrec *new, struct userrec *old, struct user_entry *e)
{
  return set_user (e->type, new, e->u.string);
}
static void
comment_display (int idx, struct user_entry *e)
{
...
struct user_entry_type USERENTRY_INFO =
  { 0, def_gotshare, def_dupuser, def_unpack, def_pack, def_write_userfile,
def_kill, def_get, def_set, def_tcl_get, def_tcl_set, def_expmem, def_display, "INFO" };
void
added_display (int idx, struct user_entry *e)
{
  if (dcc[idx].user && (dcc[idx].user->flags & USER_OWNER))
    {
      char tmp[30], tmp2[70], *hnd;
      time_t tm;
	...
	*hnd++ = 0;
      tm = atoi (tmp);
      strftime (tmp2, sizeof (tmp2), STR ("%a, %d %b %Y %H:%M:%S GMT"),
		gmtime (&tm));
      if (hnd)
...
int
added_gotshare (struct userrec *u, struct user_entry *e, char *data, int idx)
{
  return e->type->set (u, e, data);
}
struct user_entry_type USERENTRY_ADDED =
  { 0, added_gotshare, def_dupuser, def_unpack, def_pack, def_write_userfile,
def_kill, def_get, def_set, 0, 0, def_expmem, added_display, "ADDED" };
int
config_set (struct userrec *u, struct user_entry *e, void *buf)
{
...
int
config_unpack (struct userrec *u, struct user_entry *e)
{
...
int
config_pack (struct userrec *u, struct user_entry *e)
{
  struct list_type *t;
...
void
config_display (int idx, struct user_entry *e)
{
  struct xtra_key *xk;
...
int
config_gotshare (struct userrec *u, struct user_entry *e, char *buf, int idx)
{
  char *arg;
  struct xtra_key *xk;
  int l;
...
int
config_dupuser (struct userrec *new, struct userrec *old,
		struct user_entry *e)
{
...
int
config_write_userfile (FILE * f, struct userrec *u, struct user_entry *e)
{
  struct xtra_key *x;
...
int
config_kill (struct user_entry *e)
{
  struct xtra_key *x, *y;
...
int
config_expmem (struct user_entry *e)
{
  struct xtra_key *x;
  int tot = 0;
  for (x = e->u.extra; x; x = x->next)
...
struct user_entry_type USERENTRY_CONFIG =
  { 0, config_gotshare, config_dupuser, config_unpack, config_pack,
config_write_userfile, config_kill, def_get, config_set, 0, 0, config_expmem, config_display, "CONFIG" };
void
stats_add (struct userrec *u, int login, int op)
{
  char *s, s2[50];
  int sl, so;
  if (!u)
...
void
stats_display (int idx, struct user_entry *e)
{
  if (dcc[idx].user && (dcc[idx].user->flags & USER_OWNER))
    {
      char *p;
...
int
stats_gotshare (struct userrec *u, struct user_entry *e, char *data, int idx)
{
  return e->type->set (u, e, data);
}
struct user_entry_type USERENTRY_STATS =
  { 0, stats_gotshare, def_dupuser, def_unpack, def_pack, def_write_userfile,
def_kill, def_get, def_set, 0, 0, def_expmem, stats_display, "STATS" };
int
pass_set (struct userrec *u, struct user_entry *e, void *buf)
{
  char new[32];
...
static int
pass_tcl_set (Tcl_Interp * irp, struct userrec *u, struct user_entry *e,
	      int argc, char **argv)
{
...
static int
laston_pack (struct userrec *u, struct user_entry *e)
{
  char work[1024];
  struct laston_info *li;
  int l;
...
static int
laston_write_userfile (FILE * f, struct userrec *u, struct user_entry *e)
{
...
static int
laston_kill (struct user_entry *e)
{
...
static int
laston_set (struct userrec *u, struct user_entry *e, void *buf)
{
  struct laston_info *li = (struct laston_info *) e->u.extra;
  if (li != buf)
    {
...
static int
laston_tcl_get (Tcl_Interp * irp, struct userrec *u, struct user_entry *e,
		int argc, char **argv)
{
  struct laston_info *li = (struct laston_info *) e->u.extra;
  char number[20];
...
static int
laston_tcl_set (Tcl_Interp * irp, struct userrec *u, struct user_entry *e,
		int argc, char **argv)
{
  struct laston_info *li;
  struct chanuserrec *cr;
...
static int
laston_expmem (struct user_entry *e)
{
  return sizeof (struct laston_info) +
    strlen (((struct laston_info *) (e->u.extra))->lastonplace) + 1;
} static int
laston_dupuser (struct userrec *new, struct userrec *old,
		struct user_entry *e)
{
...
struct user_entry_type USERENTRY_LASTON =
  { 0, 0, laston_dupuser, laston_unpack, laston_pack, laston_write_userfile,
laston_kill, def_get, laston_set, laston_tcl_get, laston_tcl_set, laston_expmem, 0, "LASTON" };
static int
botaddr_unpack (struct userrec *u, struct user_entry *e)
{
  char p[1024], *q1, *q2;
...
static int
botaddr_pack (struct userrec *u, struct user_entry *e)
{
  char work[1024];
  struct bot_addr *bi;
  int l;
  ...
  l =
    simple_sprintf (work, STR ("%s:%u/%u:%u:%s"), bi->address,
		    bi->telnet_port, bi->relay_port, bi->hublevel,
		    bi->uplink);
  e->u.list = user_malloc (sizeof (struct list_type));
  e->u.list->next = NULL;
  e->u.list->extra = user_malloc (l + 1);
...
static int
botaddr_kill (struct user_entry *e)
{
  Context;
...
static int
botaddr_write_userfile (FILE * f, struct userrec *u, struct user_entry *e)
{
...
static int
botaddr_set (struct userrec *u, struct user_entry *e, void *buf)
{
  register struct bot_addr *bi = (struct bot_addr *) e->u.extra;
  Context;
  if (!bi && !buf)
  ...
  if (bi && !noshare && !(u->flags & USER_UNSHARED))
    {
      shareout (NULL, STR ("c BOTADDR %s %s %d %d %d %s\n"), u->handle,
		(bi->address
		 && bi->address[0]) ? bi->address : STR ("127.0.0.1"),
		bi->telnet_port, bi->relay_port, bi->hublevel, bi->uplink);
    }
  return 1;
}
static int
botaddr_tcl_get (Tcl_Interp * interp, struct userrec *u, struct user_entry *e,
		 int argc, char **argv)
{
  register struct bot_addr *bi = (struct bot_addr *) e->u.extra;
  char number[20];
...
static int
botaddr_tcl_set (Tcl_Interp * irp, struct userrec *u, struct user_entry *e,
		 int argc, char **argv)
{
  register struct bot_addr *bi = (struct bot_addr *) e->u.extra;
...
static int
botaddr_expmem (struct user_entry *e)
{
  register struct bot_addr *bi = (struct bot_addr *) e->u.extra;
  Context;
  return strlen (bi->address) + 1 + strlen (bi->uplink) + 1 +
    sizeof (struct bot_addr);
} static void
botaddr_display (int idx, struct user_entry *e)
{
#ifdef HUB
...
botaddr_gotshare (struct userrec *u, struct user_entry *e, char *buf, int idx)
{
  struct bot_addr *bi = user_malloc (sizeof (struct bot_addr));
  char *arg;
  bzero (bi, sizeof (struct bot_addr));
...
static int
botaddr_dupuser (struct userrec *new, struct userrec *old,
		 struct user_entry *e)
{
...
struct user_entry_type USERENTRY_BOTADDR =
  { 0, botaddr_gotshare, botaddr_dupuser, botaddr_unpack, botaddr_pack,
botaddr_write_userfile, botaddr_kill, def_get, botaddr_set, botaddr_tcl_get, botaddr_tcl_set, botaddr_expmem,
botaddr_display, "BOTADDR" };
int
xtra_set (struct userrec *u, struct user_entry *e, void *buf)
{
  struct xtra_key *curr, *old = NULL, *new = buf;
  for (curr = e->u.extra; curr; curr = curr->next)
    {
  ...
  struct user_entry_type USERENTRY_XTRA =
    { 0, xtra_gotshare, xtra_dupuser, xtra_unpack, xtra_pack,
xtra_write_userfile, xtra_kill, def_get, xtra_set, xtra_tcl_get, xtra_tcl_set, xtra_expmem, xtra_display,
"XTRA" };
  ...
  struct user_entry_type USERENTRY_HOSTS =
    { 0, hosts_gotshare, hosts_dupuser, hosts_null, hosts_null,
hosts_write_userfile, hosts_kill, def_get, hosts_set, hosts_tcl_get, hosts_tcl_set, hosts_expmem,
hosts_display, "HOSTS" };
