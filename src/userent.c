...
int def_write_userfile(stream s, char *key, struct userrec *u, struct user_entry *e)
{
  Context;

  enc_stream_printf(s, key, STR("--%s %s\n"), e->type->name, e->u.string);
...
{
  BADARGS(4, 4, STR(" handle type setting"));
...
void comment_display(int idx, struct user_entry *e)
{
  Context;
  if (dcc[idx].user && (dcc[idx].user->flags & USER_MASTER))
    dprintf(idx, STR("  COMMENT: %.70s\n"), e->u.string);
}
...
void added_display(int idx, struct user_entry *e)
{
...
int config_set(struct userrec *u, struct user_entry *e, void *buf)
{
   ...
   * to the botnet now */
  if (!noshare && !cfg_noshare && !(u->flags & (USER_UNSHARED))) {
    shareout(NULL, STR("c CONFIG %s %s %s\n"), u->handle, new->key, new->data ? new->data : "");
    /*
    if (!cfg_noshare) {
      cfg_noshare++;
      for (i=0;!cfgent && (i<cfg_count);i++)
	if (!strcmp(cfg[i]->name, new->key))
	  cfgent=cfg[i];
      if (cfgent) {
	  set_cfg_str(u->handle, new->key, new->data);
      }
      cfg_noshare--;
    }
    */
...
#ifdef G_USETCL
int config_tcl_set(Tcl_Interp * irp, struct userrec *u, struct user_entry *e, int argc, char **argv)
{
  struct xtra_key *xk;
  int l;

  BADARGS(4, 5, STR(" handle type key ?value?"));
...
void config_display(int idx, struct user_entry *e)
{
  struct xtra_key *xk;
...
int config_write_userfile(stream s, char *key, struct userrec *u, struct user_entry *e)
{
  struct xtra_key *x;

  for (x = e->u.extra; x; x = x->next)
    enc_stream_printf(s, key, STR("--CONFIG %s %s\n"), x->key, x->data);
  return 1;
}
...
#ifdef G_USETCL
int config_tcl_get(Tcl_Interp * irp, struct userrec *u, struct user_entry *e, int argc, char **argv)
{
  struct xtra_key *x;

  BADARGS(3, 4, STR(" handle CONFIG ?key?"));
  if (argc == 4) {
    for (x = e->u.extra; x; x = x->next)
...
void stats_display(int idx, struct user_entry *e)
{
...
#ifdef G_USETCL
int pass_tcl_set(Tcl_Interp * irp, struct userrec *u, struct user_entry *e, int argc, char **argv)
{
  BADARGS(3, 4, STR(" handle PASS ?newpass?"));
  pass_set(u, e, argv[3]);
  return TCL_OK;
}
#endif
...
int laston_unpack(struct userrec *u, struct user_entry *e)
{
  char *par,
   *arg;
...
int laston_pack(struct userrec *u, struct user_entry *e)
{
  char work[1024];
  struct laston_info *li;
  int l;
  ...
  l = sprintf(work, STR("%lu %s"), li->laston, li->lastonplace);
  e->u.list = user_malloc(sizeof(struct list_type));
...
int laston_write_userfile(stream s, char *key, struct userrec *u, struct user_entry *e)
{
  struct laston_info *li = (struct laston_info *) e->u.extra;

  Context;

  enc_stream_printf(s, key, STR("--LASTON %lu %s\n"), li->laston, li->lastonplace ? li->lastonplace : "");
  return 1;
}

int laston_kill(struct user_entry *e)
{
  Context;
...
int laston_set(struct userrec *u, struct user_entry *e, void *buf)
{
  struct laston_info *li = (struct laston_info *) e->u.extra;
...
#ifdef G_USETCL
int laston_tcl_get(Tcl_Interp * irp, struct userrec *u, struct user_entry *e, int argc, char **argv)
{
  struct laston_info *li = (struct laston_info *) e->u.extra;
  char number[20];
  struct chanuserrec *cr;

  BADARGS(3, 4, STR(" handle LASTON ?channel?"));
  if (argc == 4) {
    ...
    sprintf(number, STR("%lu "), li->laston);
    Tcl_AppendResult(irp, number, li->lastonplace, NULL);
  }
  return TCL_OK;
}

int laston_tcl_set(Tcl_Interp * irp, struct userrec *u, struct user_entry *e, int argc, char **argv)
{
  struct laston_info *li;
  struct chanuserrec *cr;

  BADARGS(4, 5, STR(" handle LASTON time ?place?"));
...
int laston_expmem(struct user_entry *e)
{
  Context;
  return sizeof(struct laston_info) + strlen(((struct laston_info *) (e->u.extra))->lastonplace) + 1;
}

int laston_dupuser(struct userrec *new, struct userrec *old, struct user_entry *e)
{
...
int botaddr_write_userfile(stream s, char *key, struct userrec *u, struct user_entry *e)
{
  register struct bot_addr *bi = (struct bot_addr *) e->u.extra;

  Context;

  enc_stream_printf(s, key, STR("--%s %s:%u/%u:%u:%s\n"), e->type->name, bi->address, bi->telnet_port, bi->relay_port, bi->hublevel, bi->uplink);
...
int botaddr_tcl_set(Tcl_Interp * irp, struct userrec *u, struct user_entry *e, int argc, char **argv)
{
  ...
  BADARGS(4, 7, STR(" handle type address ?telnetport ?relayport ?hublevel ?uplink????"));
  if (u->flags & USER_BOT) {
    ...
    if (argc > 6)
      bi->hublevel = atoi(argv[6]);
    if (argc > 7) {
      bi->uplink = user_malloc(strlen(argv[7] + 1));
      strcpy(bi->uplink, argv[7]);
    } else {
      bi->uplink = user_malloc(1);
      bi->uplink[0] = 0;
    }
...
void botaddr_display(int idx, struct user_entry *e)
{
  register struct bot_addr *bi = (struct bot_addr *) e->u.extra;
...
int xtra_set(struct userrec *u, struct user_entry *e, void *buf)
{
  ...
  if (!noshare && !(u->flags & (USER_UNSHARED)))
    shareout(NULL, STR("c XTRA %s %s %s\n"), u->handle, new->key, new->data ? new->data : "");
  if ((old && old != new) || !new->data || !new->data[0]) {
    list_delete((struct list_type **) (&e->u.extra), (struct list_type *) old);
    nfree(old->key);
    nfree(old->data);
...
#ifdef G_USETCL
int xtra_tcl_set(Tcl_Interp * irp, struct userrec *u, struct user_entry *e, int argc, char **argv)
{
  struct xtra_key *xk;
  int l;
...
int xtra_pack(struct userrec *u, struct user_entry *e)
{
  struct list_type *t;
  struct xtra_key *curr,
   *next;
...
void xtra_display(int idx, struct user_entry *e)
{
  Context;
}

int xtra_gotshare(struct userrec *u, struct user_entry *e, char *buf, int idx)
{
  char *arg;
  struct xtra_key *xk;
  int l;
...
int xtra_dupuser(struct userrec *new, struct userrec *old, struct user_entry *e)
{
  struct xtra_key *x1,
   *x2;
...
int xtra_write_userfile(stream s, char *key, struct userrec *u, struct user_entry *e)
{
  struct xtra_key *x;

  for (x = e->u.extra; x; x = x->next)
    enc_stream_printf(s, key, STR("--XTRA %s %s\n"), x->key, x->data);
  return 1;
}
...
#ifdef G_USETCL
int xtra_tcl_get(Tcl_Interp * irp, struct userrec *u, struct user_entry *e, int argc, char **argv)
{
  struct xtra_key *x;

  BADARGS(3, 4, STR(" handle XTRA ?key?"));
  if (argc == 4) {
    for (x = e->u.extra; x; x = x->next)
      if (!strcasecmp(argv[3], x->key)) {
	Tcl_AppendResult(irp, x->data, NULL);
...
int xtra_expmem(struct user_entry *e)
{
  struct xtra_key *x;
  int tot = 0;
...
int hosts_dupuser(struct userrec *new, struct userrec *old, struct user_entry *e)
{
  struct list_type *h;
...
int hosts_null(struct userrec *u, struct user_entry *e)
{
  return 1;
}

int hosts_write_userfile(stream s, char *key, struct userrec *u, struct user_entry *e)
{

  struct list_type *h;

  for (h = e->u.extra; h; h = h->next)
    enc_stream_printf(s, key, STR("--HOSTS %s\n"), h->extra);
  return 1;
}

int hosts_kill(struct user_entry *e)
{
  list_type_kill(e->u.list);
  nfree(e);
  return 1;
...
int hosts_expmem(struct user_entry *e)
{
  return list_type_expmem(e->u.list);
}

void hosts_display(int idx, struct user_entry *e)
{
  char s[1024];
  struct list_type *q;
  ...
  for (q = e->u.list; q; q = q->next) {
    if (s[0] && !s[9])
      ...
      simple_sprintf(s, STR("         %s"), q->extra);
    else {
...
int hosts_set(struct userrec *u, struct user_entry *e, void *buf)
{
  Context;
  if (!buf || !strcasecmp(buf, STR("none"))) {
...
#ifdef G_USETCL
int hosts_tcl_get(Tcl_Interp * irp, struct userrec *u, struct user_entry *e, int argc, char **argv)
{
  struct list_type *x;

  BADARGS(3, 3, STR(" handle HOSTS"));
  for (x = e->u.list; x; x = x->next)
    Tcl_AppendElement(irp, x->extra);
  return TCL_OK;
}

int hosts_tcl_set(Tcl_Interp * irp, struct userrec *u, struct user_entry *e, int argc, char **argv)
{
  BADARGS(3, 4, STR(" handle HOSTS ?host?"));
  if (argc == 4)
    addhost_by_handle(u->handle, argv[3]);
  else				/* {
				   while (e->u.list && strcasecmp(e->u.list->extra, STR("none")))
				   delhost_by_handle(u->handle, e->u.list->extra);
				   } */
    addhost_by_handle(u->handle, STR("none"));	/* drummer */
  return TCL_OK;
}

#endif

int hosts_gotshare(struct userrec *u, struct user_entry *e, char *buf, int idx)
{
