...
#endif
void *
_user_malloc (int size, const char *file, int line)
{
#ifdef DEBUG_MEM
  char x[1024];
  const char *p;
...
void *
_user_realloc (void *ptr, int size, const char *file, int line)
{
#ifdef DEBUG_MEM
  char x[1024];
  const char *p;
...
struct userrec *
check_dcclist_hand (char *handle)
{
  int i;
...
struct userrec *
get_user_by_handle (struct userrec *bu, char *handle)
{
...
void
clear_userlist (struct userrec *bu)
{
  struct userrec *u, *v;
  int i;
  for (u = bu; u; u = v)
...
struct userrec *
get_user_by_equal_host (char *host)
{
  struct userrec *u;
  struct list_type *q;
  for (u = userlist; u; u = u->next)
...
int
u_pass_match (struct userrec *u, char *pass)
{
  char *cmp, new[32];
  if (!u)
    return 0;
...
int
write_user (struct userrec *u, FILE * f, int idx)
{
  char s[181];
  struct chanuserrec *ch;
  struct chanset_t *cst;
...
write_userfile (int idx)
{
  ...
  if (!ok || fflush (f))
    {
      putlog (LOG_MISC, "*", "%s (%s)", USERF_ERRWRITE,
	      strerror (ferror (f)));
      fclose (f);
      nfree (new_userfile);
      return;
...
int
change_handle (struct userrec *u, char *newh)
{
  int i;
  char s[HANDLEN + 1];
  if (!u)
...
struct userrec *
adduser (struct userrec *bu, char *handle, char *host, char *pass, int flags)
{
  struct userrec *u, *x;
...
int
delhost_by_handle (char *handle, char *host)
{
  struct userrec *u;
...
void
addhost_by_handle (char *handle, char *host)
{
...
void
touch_laston (struct userrec *u, char *where, time_t timeval)
{
  if (!u)
    return;
  if (timeval > 1)
