...
void
dcc_dnswait (int idx, char *buf, int len)
{
} void
eof_dcc_dnswait (int idx)
{
...
display_dcc_dnswait (int idx, char *buf)
{
  sprintf (buf, "dns   waited %lus", now - dcc[idx].timeval);
} static int
expmem_dcc_dnswait (void *x)
{
...
static void
kill_dcc_dnswait (int idx, void *x)
{
  register struct dns_info *p = (struct dns_info *) x;
  if (p)
    {
...
struct dcc_table DCC_DNSWAIT =
  { "DNSWAIT", DCT_VALIDIDX, eof_dcc_dnswait, dcc_dnswait, 0, 0,
display_dcc_dnswait, expmem_dcc_dnswait, kill_dcc_dnswait, 0 };
static void
dns_dcchostbyip (IP ip, char *hostn, int ok, void *other)
{
  int idx;
...
static void
dns_dccipbyhost (IP ip, char *hostn, int ok, void *other)
{
  int idx;
  for (idx = 0; idx < dcc_total; idx++)
    {
...
static int
dns_dccexpmem (void *other)
{
  return 0;
}
devent_type DNS_DCCEVENT_HOSTBYIP =
  { "DCCEVENT_HOSTBYIP", dns_dccexpmem, dns_dcchostbyip };
devent_type DNS_DCCEVENT_IPBYHOST =
  { "DCCEVENT_IPBYHOST", dns_dccexpmem, dns_dccipbyhost };
void
dcc_dnsipbyhost (char *hostn)
{
  devent_t *de;
...
static void
dns_tcl_iporhostres (IP ip, char *hostn, int ok, void *other)
{
  devent_tclinfo_t *tclinfo = (devent_tclinfo_t *) other;
  if (Tcl_VarEval
      (interp, tclinfo->proc, " ", iptostr (htonl (ip)), " ", hostn,
...
devent_type DNS_TCLEVENT_HOSTBYIP =
  { "TCLEVENT_HOSTBYIP", dns_tclexpmem, dns_tcl_iporhostres };
devent_type DNS_TCLEVENT_IPBYHOST =
  { "TCLEVENT_IPBYHOST", dns_tclexpmem, dns_tcl_iporhostres };
static void
tcl_dnsipbyhost (char *hostn, char *proc, char *paras)
{
  devent_t *de;
...
static void
tcl_dnshostbyip (IP ip, char *proc, char *paras)
{
  devent_t *de;
  devent_tclinfo_t *tclinfo;
  de = nmalloc (sizeof (devent_t));
...
void
call_hostbyip (IP ip, char *hostn, int ok)
{
	  ...
	  && (!de->res_data.ip_addr || (de->res_data.ip_addr == ip)))
	{
	  ...
	  if (de->type && de->type->event)
	    ...
	    putlog (LOG_MISC, "*", "(!) Unknown DNS event type found: %s",
		    (de->type
		     && de->type->name) ? de->type->name : "<empty>");
	  nfree (de);
	  de = ode;
	}
      ode = de;
...
void
call_ipbyhost (char *hostn, IP ip, int ok)
{
  devent_t *de = dns_events, *ode = NULL, *nde = NULL;
  while (de)
    {
...
tcl_cmds tcldns_cmds[] = { {"dnslookup", tcl_dnslookup}
, {NULL, NULL}
};
