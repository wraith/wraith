...
void
addignore (char *ign, char *from, char *mnote, time_t expire_time)
{
...
void
display_ignore (int idx, int number, struct igrec *ignore)
{
  char dates[81], s[41];
  if (ignore->added)
    {
...
void
tell_ignores (int idx, char *match)
{
  struct igrec *u = global_ign;
  int k = 1;
  if (u == NULL)
  ...
  for (; u; u = u->next)
    {
      if (match[0])
	{
	  if (wild_match (match, u->igmask) || wild_match (match, u->msg)
...
static void
addmask_fully (struct chanset_t *chan, maskrec ** m, maskrec ** global,
	       char *mask, char *from, char *note, time_t expire_time,
	       int flags, time_t added, time_t last)
{
...
restore_chanban (struct chanset_t *chan, char *host)
{
      ...
      if (add)
	{
	  ...
	  if (*add == '+')
		  ...
		  *last = 0;
		  ...
		  if (user)
		      ...
		      if (desc)
			{
			  *desc = 0;
			  desc++;
			  addmask_fully (chan, &chan->bans, &global_bans,
					 host, user, desc, atoi (expi), flags,
					 atoi (add), atoi (last));
			  return;
			}
		    }
	      ...
	      if (desc)
		{
		  *desc = 0;
		  desc++;
		  addmask_fully (chan, &chan->bans, &global_bans, host, add,
...
static void
restore_chanexempt (struct chanset_t *chan, char *host)
{
  char *expi, *add, *last, *user, *desc;
  int flags = 0;
  expi = strchr_unescape (host, ':', '\\');
      ...
      if (add)
	{
	  ...
	  if (*add == '+')
		  ...
		  *last = 0;
		  ...
		  if (user)
		      ...
		      if (desc)
			{
			  *desc = 0;
			  desc++;
			  addmask_fully (chan, &chan->exempts,
					 &global_exempts, host, user, desc,
					 atoi (expi), flags, atoi (add),
					 atoi (last));
			  return;
			}
		    }
	      ...
	      if (desc)
		{
		  *desc = 0;
		  desc++;
		  addmask_fully (chan, &chan->exempts, &global_exempts, host,
...
static void
restore_chaninvite (struct chanset_t *chan, char *host)
{
  char *expi, *add, *last, *user, *desc;
  int flags = 0;
  expi = strchr_unescape (host, ':', '\\');
      ...
      if (add)
	{
	  ...
	  if (*add == '+')
		  ...
		  *last = 0;
		  ...
		  if (user)
		      ...
		      if (desc)
			{
			  *desc = 0;
			  desc++;
			  addmask_fully (chan, &chan->invites,
					 &global_invites, host, user, desc,
					 atoi (expi), flags, atoi (add),
					 atoi (last));
			  return;
			}
	      ...
	      if (desc)
		{
		  *desc = 0;
		  desc++;
		  addmask_fully (chan, &chan->invites, &global_invites, host,
				 add, desc, atoi (expi), flags, now, 0);
		  return;
		}
	    }
...
void
tell_user (int idx, struct userrec *u, int master)
{
  char s[81], s1[81];
  char format[81];
  int n;
  ...
  egg_snprintf (format, sizeof format,
		"%%-%us %%-5s%%5d %%-15s %%s (%%-10.10s)", HANDLEN);
  dprintf (idx, format, u->handle,
	   get_user (&USERENTRY_PASS, u) ? "yes" : "no", n, s, s1, (li
								    && li->
								    lastonplace)
	   ? li->lastonplace : "nowhere");
  dprintf (idx, "\n");
...
void
tell_user_ident (int idx, char *id, int master)
{
  char format[81];
  struct userrec *u;
...
void
tell_users_match (int idx, char *mtch, int start, int limit, int master,
		  char *chname)
{
  char format[81];
  struct userrec *u;
...
int
readuserfile (char *file, struct userrec **ret)
{
  char *p, buf[1024], lasthand[512], *attr, *pass, *code, s1[1024], *s,
    cbuf[1024], *temps;
  char *horesak;
  FILE *f;
	  ...
	  if (s[0] != '#' && s[0] != ';' && s[0])
	      ...
	      if (!strcmp (code, "-"))
		{
		  ...
		  if (s[0])
		    {
		      if (lasthand[0]
			  && strchr (CHANMETA, lasthand[0]) != NULL)
	      ...
	      else if (!strncmp (code, "--", 2))
		{
		  if (u)
		      ...
		      if (!ok)
			{
			  ...
			  ue->u.list =
			    user_malloc (sizeof (struct list_type));
			  ue->u.list->next = NULL;
		  ...
		  if (!attr[0] || !pass[0])
		      ...
		      else
			{
			  ...
			  if (strlen (pass) > 20)
			    {
			      putlog (LOG_MISC, "*", "* %s '%s'",
				      USERF_BROKEPASS, code);
			      strcpy (pass, "-");
			    }
			  bu =
			    adduser (bu, code, 0, pass,
...
void
link_pref_val (struct userrec *u, char *val)
{
  struct bot_addr *ba;
  val[0] = 'Z';
  val[1] = 0;
...
struct userrec *
next_hub (struct userrec *current, char *lowval, char *highval)
{
  char thisval[NICKLEN + 4], bestmatchval[NICKLEN + 4] =
    "z", bestallval[NICKLEN + 4] = "z";
  ...
  while (cur != current)
      ...
      if ((cur->flags & USER_BOT) && (strcmp (cur->handle, botnetnick)))
	{
	  ...
	  if ((strcmp (thisval, lowval) < 0)
	      && (strcmp (thisval, highval) > 0)
	      && (strcmp (thisval, bestmatchval) < 0))
	    {
...
autolink_cycle (char *start)
{
  ...
  for (i = 0; i < dcc_total; i++)
      ...
      if (dcc[i].type == &DCC_BOT)
	{
	  ...
	  if (dcc[i].u.bot->numver != egg_numver)
	    continue;
	  if (dcc[i].
	      status & (STAT_SHARE | STAT_OFFERED | STAT_SENDING |
			STAT_GETTING))
	    {
		  ...
		  if (dcc[i].status & STAT_AGGRESSIVE)
		    {
		      putlog (LOG_MISC, "*",
			      STR
			      ("Passively sharing with %s but should be aggressive"),
			      dcc[i].user->handle);
		      putlog (LOG_DEBUG, "*",
			      STR ("My linkval: %s - %s linkval: %s"), myval,
			      dcc[i].nick, curval);
		      botunlink (-2, dcc[i].nick,
				 STR
				 ("Marked passive, should be aggressive"));
		      return;
		    }
		}
	      else
		  ...
		  if (!(dcc[i].status & STAT_AGGRESSIVE))
		    {
		      putlog (LOG_MISC, "*",
			      STR
			      ("Aggressively sharing with %s but should be passive"),
			      dcc[i].user->handle);
		      putlog (LOG_DEBUG, "*",
			      STR ("My linkval: %s - %s linkval: %s"), myval,
			      dcc[i].nick, curval);
		      botunlink (-2, dcc[i].nick,
				 STR
				 ("Marked aggressive, should be passive"));
		      return;
		    }
  ...
  if (u)
      ...
      if (strcmp (bestval, curval) < 0)
	{
	  putlog (LOG_BOTS, "*",
		  STR
		  ("Failed link attempt to %s but connected to %s already???"),
		  u->handle, (char *) &bestval[3]);
	  return;
	}
    }
  else
  ...
  while (ul)
    {
      ...
      if (glob_bot (fr) && strcmp (ul->handle, botnetnick)
	  && strcmp (u->handle, avoidbot) && (bot_hublevel (ul) < 999))
	{
	  hl2 = hl;
