...
static int tcl_console STDVAR
{
  ...
  for (arg = 2; arg < argc; arg++)
    {
      if (argv[arg][0]
	  && ((strchr (CHANMETA, argv[arg][0]) != NULL)
	      || (argv[arg][0] == '*')))
	{
	...
	do_console_flags:for (j = 0; j < strlen (argv[arg]); j++)
	    {
	      if (argv[arg][j] == '+')
		pls = 1;
	      else if (argv[arg][j] == '-')
  ...
  if (dcc[idx].type->flags & DCT_CHAT)
    {
      chanout_but (idx, dcc[idx].u.chat->channel,
		   "*** %s has left the %s%s%s\n", dcc[idx].nick,
		   dcc[idx].u.chat ? "channel" : "partyline",
  ...
  *buf = 0;
  ...
  while (fgets (templine, sizeof (templine), f))
      ...
      if (strchr (templine, '\n'))
	{
      ...
      strcpy (check, temps2);
      line++;
      if (!strncmp (check, "-ifhub", 6) || !strncmp (check, "-ifleaf", 7)
	  || !strncmp (check, "-endif", 6) || (!strncmp (check, "-else", 5)
					       && (skip == 1 || skip == 2))
	  || !strncmp (check, "/*", 2) || !strncmp (check, "*/", 2)
	  || !strcmp (check + (strlen (check) - 2), "*/")
	  || !strncmp (check, "//", 2))
	{
	  if (!strncmp (check, "-", 1) && !incom)
	    {
	      if (!strncmp (check, "-ifhub", 6))
		skip = 1;
	      else if (!strncmp (check, "-ifleaf", 7))
		skip = 2;
	      else if (!strncmp (check, "-endif", 6))
		{
		  if (skip)
		    skip = 0;
		  else
		    {
		      putlog (LOG_MISC, "*",
			      "Error on line %i -endif with no ifdef!", line);
		      return 0;
		    }
		}
	      else if (!strncmp (check, "-else", 5))
		{
		  if (skip == 1)
		    skip = 2;
		  else if (skip == 2)
		    ...
		    {
		      putlog (LOG_MISC, "*",
			      "Error on line %i -else with no ifdef!", line);
		      return 0;
		    }
		}
	    }
		...
		if (!incom)
		  ...
		  {
		    putlog (LOG_MISC, "*",
			    "Error on line %i New /* inside a /*", line);
		    return 0;
		  }
	      else if (!strcmp (check + (strlen (check) - 2), "*/")
		       || !strncmp (check, "*/", 2))
		{
		  if (incom)
		    incom = 0;
		    ...
		    {
		      putlog (LOG_MISC, "*",
			      "Error on line %i */ without a /*", line);
		      return 0;
		    }
		}
	    }
	...
	{
	  if ((((hub && (skip == 1)) || (leaf && (skip == 2))) || (!skip))
	      && !incom)
	    {
	      lines++;
	      strcat (buf, temps2);
  ...
  if (argv[1][0] == '*')
      ...
      if ((chan < 0) || (chan > 199999))
	{
	  Tcl_AppendResult (irp,
			    "channel out of range; must be 0 thru 199999",
			    NULL);
	  return TCL_ERROR;
	}
    }
	...
	if (dcc[i].u.chat->channel == chan || chan == -1)
	  {
	    c[0] = geticon (i);
	    c[1] = 0;
	    egg_snprintf (idle, sizeof idle, "%lu",
			  (now - dcc[i].timeval) / 60);
	    list[0] = dcc[i].nick;
	    list[1] = botnetnick;
	    list[2] = dcc[i].host;
	    list[3] = c;
	    ...
	    if (chan == -1)
	      {
		egg_snprintf (work, sizeof work, "%d",
...
static int tcl_listen STDVAR
{
  ...
  if (idx < 0)
	...
	{
	  Tcl_AppendResult (irp,
			    "Couldn't assign the requested IP. Please make sure 'my-ip' is set properly.",
			    NULL);
	  return TCL_ERROR;
	}
  ...
  if (!strcmp (argv[2], "script"))
    {
      ...
      if (argc < 4)
	{
	  Tcl_AppendResult (irp, "must give proc name for script listen",
			    NULL);
	  killsock (dcc[idx].sock);
	  lostdcc (idx);
	  return TCL_ERROR;
      ...
      if (argc == 5)
	{
	  if (strcmp (argv[4], "pub"))
	    {
	      Tcl_AppendResult (irp, "unknown flag: ", argv[4],
				". allowed flags: pub", NULL);
	      killsock (dcc[idx].sock);
	      lostdcc (idx);
	      return TCL_ERROR;
	    }
...
static int tcl_traffic STDVAR
{
  ...
  unsigned long in_total_today, in_total;
    ...
    itraffic_trans_today + itraffic_unknown_today, in_total =
    in_total_today + itraffic_irc + itraffic_bn + itraffic_dcc +
    itraffic_trans + itraffic_unknown;
    out_total_today =
    otraffic_irc_today + otraffic_bn_today + otraffic_dcc_today +
    itraffic_trans_today + otraffic_unknown_today, out_total =
    out_total_today + otraffic_irc + otraffic_bn + otraffic_dcc +
...
tcl_cmds tcldcc_cmds[] = { {"binname", tcl_binname}
, {"putdcc", tcl_putdcc}
