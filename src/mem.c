...
#define MAX_MEM 13
  ...
  for (i = 0; i < MAX_MEM; i++)
	...
	{
	  dprintf (idx,
		   "File '%-10s' accounted for %lu/%lu (debug follows:)\n",
		   fn, exp[i], use[i]);
	  strcpy (sofar, "   ");
  ...
  for (me = module_list; me; me = me->next)
	...
	{
	  dprintf (idx,
		   "Module '%-10s' accounted for %lu/%lu (debug follows:)\n",
		   me->name, expt, me->mem_work);
	  strcpy (sofar, "   ");
	  for (j = 0; j < lastused; j++)
	    {
	      ...
	      if ((p = strchr (fn, ':')) != NULL)
		{
		  *p = 0;
		  if (!egg_strcasecmp (fn, me->name))
		    {
		      sprintf (&sofar[strlen (sofar)], "%-10s/%-4d:(%04X) ",
			       p + 1, memtbl[j].line, memtbl[j].size);
		      if (strlen (sofar) > 60)
			{
			  sofar[strlen (sofar) - 1] = 0;
...
n_malloc (int size, const char *file, int line)
{
  void *x;
#ifdef DEBUG_MEM
  int i = 0;
...
void *
n_realloc (void *ptr, int size, const char *file, int line)
{
  void *x;
  int i = 0;
#ifdef DEBUG_MEM
  ...
  if (i == lastused)
    {
      putlog (LOG_MISC, "*",
	      "*** ATTEMPTING TO REALLOC NON-MALLOC'D PTR: %s (%d)", file,
	      line);
      return NULL;
    }
...
void
n_free (void *ptr, const char *file, int line)
{
  int i = 0;
  if (ptr == NULL)
    {
  ...
  if (line)
	...
	{
	  putlog (LOG_MISC, "*",
		  "*** ATTEMPTING TO FREE NON-MALLOC'D PTR: %s (%d)", file,
		  line);
	  return;
	}
      memused -= memtbl[i].size;
