...
void check_tcl_chjn (const char *, const char *, int, char, int,
		     const char *);
...
void check_tcl_chatactbcst (const char *, int, const char *,
			    tcl_bind_list_t *);
...
#endif
#define CHECKVALIDITY(a)	do {	if (!check_validity(argv[0], (a))) {	Tcl_AppendResult(irp, "bad builtin command call!",	NULL);	return TCL_ERROR;	}	} while (0)
#endif
