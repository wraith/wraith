...
#define fixcolon(x)	do {	if ((x)[0] == ':')	(x)++;	else	(x) = newsplit(&(x));	} while (0)
#define strncpyz(_target, _source, _len)	do {	strncpy((_target), (_source), (_len) - 1);	(_target)[(_len) - 1] = 0;	} while (0)
#ifdef BORGCUBES
#define O_NONBLOCK	00000004
#endif
