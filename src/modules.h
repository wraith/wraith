...
void *mod_malloc (int size, const char *modname, const char *filename,
		  int line);
void *mod_realloc (void *ptr, int size, const char *modname,
		   const char *filename, int line);
void mod_free (void *ptr, const char *modname, const char *filename,
	       int line);
...
#define call_hook(x) do {	register struct hook_entry *p, *pn;	for (p = hook_list[x]; p; p = pn) {	pn = p->next;	p->func();	}	} while (0)
